...
1. **Read the docs that already answer most questions.**
   ...
   - [`contract/registries/capabilities.v1.json`](./contract/registries/capabilities.v1.json)
     ...
     **experimental**; the current FigureRequestV1 catalog has no 3D, knowledge-graph,
     animation, or NCP-adapter skill. None inherits stable FigureRequestV1 guarantees.
...
| A change you want to contribute | A PR against `main`, per [`CONTRIBUTING.md`](./CONTRIBUTING.md) | Cortexel is the canonical, writable source; downstream consumers pin reviewed commits by full SHA. |

When filing an issue, include the exact package/build identity from `cortexel identity --json`, the entrypoint you called
...
- **It cannot promise legacy or absent capabilities behave like stable FigureRequestV1
  skills.** The packaged legacy 3D spatial, knowledge-graph, and animation surfaces are
  GPU/driver- or layout-dependent, and only the knowledge-graph export is registered as
  an available experimental package capability. No current NCP adapter capability
  exists. Issues about legacy surfaces are welcome, but they do not inherit the stable
  FigureRequestV1 determinism, byte-stable-export, or accessibility guarantees.
...
it is the reason the current line is labeled a preview rather than a stable release.
