...
var normalizedRecordKey = z.string().min(1).max(80).regex(
  /^\S(?:[\s\S]*\S)?$/,
...
var JsonParamsSchema = z.unknown().transform((params, ctx) => {
...
var timeArray = z3.array(z3.number()).max(PARAM_LIMITS.maxSamples);
var gpuNumber = z3.number().min(-FLOAT32_MAX, "value exceeds the finite Float32 range used by render buffers").max(FLOAT32_MAX, "value exceeds the finite Float32 range used by render buffers");
var gpuArray = z3.array(gpuNumber).max(PARAM_LIMITS.maxSamples);
var idArray = z3.array(
  z3.number().int("node/sender ids must be integers").nonnegative("node/sender ids must be non-negative").max(Number.MAX_SAFE_INTEGER, "node/sender ids must be safe integers")
).max(PARAM_LIMITS.maxSamples);
var displayText = (max) => z3.string().trim().min(1).max(max).regex(SAFE_DISPLAY_STRING_PATTERN, "display text must not contain control or bidi characters").meta({ "x-cortexel-normalize": "trim" });
var Rfc3339TimestampSchema = z3.iso.datetime({ offset: true }).max(80).regex(
...
var normalizedRecordKey2 = z3.string().min(1).max(80).regex(
  /^\S(?:[\s\S]*\S)?$/,
  "record keys must already be trimmed and contain a non-whitespace character"
...
var SpikeRasterParamsSchema = z3.object({
...
var IsiDistributionParamsSchema = z3.object({
  bin_centers_ms: timeArray.min(1),
  values: gpuArray.min(1),
  bin_width_ms: z3.number().positive().max(Number.MAX_VALUE),
  normalization: z3.enum(["count", "probability", "probability_density"]),
  value_units: z3.enum(["count", "probability", "1/ms"]),
  interval_scope: z3.enum(["per_sender", "single_train"])
}).strict().superRefine((value, ctx) => {
  equalLengthIssue(
    ctx,
    "values",
...
var PsthParamsSchema = z3.object({
  bin_centers_ms: timeArray.min(1),
  values: gpuArray.min(1),
  bin_width_ms: z3.number().positive().max(Number.MAX_VALUE),
  normalization: z3.enum(["count", "count_per_trial", "rate_hz"]),
  value_units: z3.enum(["count", "count/trial", "Hz"]),
  trial_count: z3.number().int().positive().max(Number.MAX_SAFE_INTEGER),
  alignment_event: displayText(240),
  aggregation: z3.literal("selected_senders_per_trial")
}).strict().superRefine((value, ctx) => {
  equalLengthIssue(
    ctx,
    "values",
...
var PopulationRateSeriesSchema = z3.object({
  id: displayText(120),
  label: displayText(240),
  recorded_sender_count: z3.number().int("recorded_sender_count must be an integer").positive("recorded_sender_count must be positive").max(Number.MAX_SAFE_INTEGER, "recorded_sender_count must be a safe integer"),
  spike_counts: z3.array(
    z3.number().int("spike counts must be integers").nonnegative("spike counts cannot be negative").max(Number.MAX_SAFE_INTEGER, "spike counts must be safe integers")
...
var PopulationRateParamsSchema = z3.object({
  bin_centers_ms: timeArray.min(1),
  bin_width_ms: z3.number().positive().max(Number.MAX_VALUE),
  window_start_ms: z3.number(),
  window_stop_ms: z3.number(),
  series: z3.array(PopulationRateSeriesSchema).min(1).max(PARAM_LIMITS.maxSeries),
  normalization: z3.literal("mean_per_recorded_sender_hz"),
  aggregation: z3.literal("selected_senders"),
...
var RateResponseParamsSchema = z3.object({
...
var NetworkParamsSchema = z3.object({
...
var topologyCount = z3.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).refine((value) => !Object.is(value, -0), "topology counts and ranks must not be negative zero");
var topologyPositiveCount = topologyCount.positive();
var MpiTargetRankLocalScopeSchema = z3.object({
  kind: z3.literal("mpi_target_rank_local"),
  rank: topologyCount,
...
var SnapshotScopeSchema = z3.discriminatedUnion("kind", [
  z3.object({ kind: z3.literal("single_process_complete") }).strict(),
  MpiTargetRankLocalScopeSchema,
  z3.object({
    kind: z3.literal("mpi_all_ranks_merged"),
    world_size: topologyPositiveCount
  }).strict()
]);
var MpiRankLocalPositionScopeSchema = z3.object({
  kind: z3.literal("mpi_rank_local"),
  rank: topologyCount,
  world_size: topologyPositiveCount
}).strict().superRefine((value, ctx) => {
  if (value.rank >= value.world_size) {
...
var PositionScopeSchema = z3.discriminatedUnion("kind", [
  z3.object({ kind: z3.literal("single_process_complete") }).strict(),
...
var ConnectionGraphNodeSchema = z3.object({
  id: idArray.element,
  label: displayText(120)
}).strict();
var ConnectionGraphEdgeSchema = z3.object({
  id: displayText(240),
...
var ConnectionGraphParamsSchema = z3.object({
  nodes: z3.array(ConnectionGraphNodeSchema).min(1).max(PARAM_LIMITS.maxTopologyNodes),
  edges: z3.array(ConnectionGraphEdgeSchema).max(PARAM_LIMITS.maxTopologyEdges),
  weight_units: units.optional(),
  delay_units: z3.literal("ms").optional(),
  layout: z3.literal("schematic_circle"),
  parallel_edges: z3.literal("preserved"),
  self_connections: z3.literal("preserved"),
  snapshot_time_ms: z3.number().finite().nonnegative(),
  snapshot_scope: SnapshotScopeSchema,
  sample_policy: z3.enum(["complete", "deterministic_even_stride"]),
  source_connection_count: topologyCount,
  edge_identity: z3.enum(["nest_connection_identifier", "canonical_sorted_ordinal"])
}).strict().superRefine((value, ctx) => {
...
var MatrixCellBaseSchema = z3.object({
...
var matrixBaseShape = {
  ...
  axis_order: z3.literal("target_rows_source_columns"),
  absent_cell: z3.literal("no_connection"),
...
function refineSparseMatrix(value, ctx) {
  ...
  if (sourceIds.size !== value.source_ids.length) {
    ctx.addIssue({ code: z3.ZodIssueCode.custom, path: ["source_ids"], message: "source_ids must be unique" });
  }
  if (targetIds.size !== value.target_ids.length) {
    ctx.addIssue({ code: z3.ZodIssueCode.custom, path: ["target_ids"], message: "target_ids must be unique" });
  }
...
var AdjacencyMatrixParamsSchema = z3.object({
  ...matrixBaseShape,
  cells: z3.array(AdjacencyMatrixCellSchema).max(PARAM_LIMITS.maxSamples),
  display: z3.literal("binary_presence"),
  aggregation: z3.literal("any_connection")
}).strict().superRefine(refineSparseMatrix);
var WeightMatrixParamsSchema = z3.object({
  ...matrixBaseShape,
  cells: z3.array(WeightMatrixCellSchema).max(PARAM_LIMITS.maxSamples),
  weight_units: units,
  aggregation: z3.enum(["sum", "mean", "minimum", "maximum", "single_connection"])
}).strict().superRefine((value, ctx) => {
...
var DelayMatrixParamsSchema = z3.object({
  ...matrixBaseShape,
  cells: z3.array(DelayMatrixCellSchema).max(PARAM_LIMITS.maxSamples),
  delay_units: z3.literal("ms"),
  aggregation: z3.enum(["mean", "minimum", "maximum", "single_connection"])
}).strict().superRefine((value, ctx) => {
  refineSparseMatrix(value, ctx);
  if (value.aggregation === "single_connection") {
    const index = value.cells.findIndex((cell) => cell.connection_count !== 1);
...
function degreeDistributionSchema(direction) {
  return z3.object({
    degrees: z3.array(topologyCount).min(1).max(PARAM_LIMITS.maxSamples),
    node_counts: z3.array(topologyCount).min(1).max(PARAM_LIMITS.maxSamples),
    values: gpuArray.min(1),
    node_count: topologyPositiveCount,
    connection_count: topologyCount,
    direction: z3.literal(direction),
    normalization: z3.enum(["count", "probability"]),
    value_units: z3.enum(["count", "probability"]),
    edge_counting: z3.literal("each_synapse_collection_entry"),
    zero_degree_policy: z3.literal("include_declared_universe"),
    sample_policy: z3.literal("complete"),
    snapshot_time_ms: z3.number().finite().nonnegative(),
    snapshot_scope: SnapshotScopeSchema
  }).strict().superRefine((value, ctx) => {
...
var DelayDistributionParamsSchema = z3.object({
  bin_centers_ms: timeArray.min(1),
  delay_counts: z3.array(topologyCount).min(1).max(PARAM_LIMITS.maxSamples),
  values: gpuArray.min(1),
  bin_width_ms: z3.number().finite().positive(),
  window_start_ms: z3.number().finite().nonnegative(),
  window_stop_ms: z3.number().finite().positive(),
  normalization: z3.enum(["count", "probability", "probability_density"]),
  value_units: z3.enum(["count", "probability", "1/ms"]),
  delay_units: z3.literal("ms"),
...
var SpatialMap2DNodeSchema = z3.object({
  id: idArray.element,
  label: displayText(120),
  x: gpuNumber,
  y: gpuNumber
...
var SpatialMap2DParamsSchema = z3.object({
  nodes: z3.array(SpatialMap2DNodeSchema).min(1).max(PARAM_LIMITS.maxSpatialObjects),
  coordinate_units: units,
  extent: z3.tuple([gpuNumber.positive(), gpuNumber.positive()]),
  center: z3.tuple([gpuNumber, gpuNumber]),
  edge_wrap: z3.boolean(),
  position_scope: PositionScopeSchema,
  marker_size: z3.literal("fixed_screen_space")
}).strict().superRefine((value, ctx) => {
  const ids = /* @__PURE__ */ new Set();
...
var Spatial3DParamsSchema = z3.object({
  objects: z3.array(Spatial3DObjectSchema).min(1).max(PARAM_LIMITS.maxSpatialObjects),
  coordinate_units: units
}).strict();
var PlasticityParamsSchema = z3.object({
  times_ms: timeArray.min(1),
...
var PhasePlaneDerivativeTimeUnitSchema = z3.enum(["ms", "s"]);
var PhasePlaneParamsSchema = z3.object({
  grid: z3.record(normalizedRecordKey2, gpuArray.min(2)).refine((g) => Object.keys(g).length === 2, {
    message: "phase-plane grid must declare exactly two state-variable axes with at least two coordinates each"
  }),
  derivatives: z3.record(normalizedRecordKey2, gpuArray.min(1)),
  axis_units: z3.record(normalizedRecordKey2, units),
  derivative_units: z3.record(normalizedRecordKey2, units),
  derivative_time_unit: PhasePlaneDerivativeTimeUnitSchema,
  axis_order: z3.tuple([normalizedRecordKey2, normalizedRecordKey2]).refine(([first, second]) => first !== second, {
    message: "axis_order must name two distinct state variables"
  }),
  flattening: z3.literal("row-major-last-axis-fastest")
}).strict().superRefine((value, ctx) => {
...
var KnowledgeGraphAttributeValueSchema = z3.union([
...
var KnowledgeGraphEvidenceRefSchema = z3.discriminatedUnion("kind", [
  ...
  z3.object({
    ...
    page: z3.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).refine((value) => !Object.is(value, -0), "page must not be negative zero").optional(),
    locator: displayText(KNOWLEDGE_GRAPH_LIMITS.maxLocatorLength).optional(),
    excerpt: displayText(KNOWLEDGE_GRAPH_LIMITS.maxExcerptLength).optional(),
...
var KnowledgeGraphEvidenceRefsSchema = z3.array(KnowledgeGraphEvidenceRefSchema).min(1).max(KNOWLEDGE_GRAPH_LIMITS.maxEvidenceRefsPerElement).superRefine((evidence, ctx) => {
...
var KnowledgeGraphUncalibratedScoreSchema = z3.object({
  kind: z3.enum([
  ...
  value: z3.number().min(0).max(1),
  calibrated_posterior: z3.literal(false)
}).strict();
var DerivedAdvisoryEpistemicSchema = z3.object({
  status: z3.literal("derived_advisory"),
  advisory_only: z3.literal(true),
  is_paper_local_evidence: z3.literal(false),
  calibrated_posterior: z3.literal(false)
}).strict();
...
var KnowledgeGraph3DParamsSchema = z3.object({
  graph_id: displayText(160),
  graph_source: displayText(200),
  graph_snapshot_id: displayText(200),
  graph_scope: z3.literal("corpus_entity"),
  generated_at: Rfc3339TimestampSchema,
  nodes: z3.array(KnowledgeGraphNodeSchema).min(1).max(PARAM_LIMITS.maxGraphNodes),
  edges: z3.array(KnowledgeGraphEdgeSchema).max(PARAM_LIMITS.maxGraphEdges)
}).strict().superRefine((value, ctx) => {
  const ids = /* @__PURE__ */ new Set();
...
var Spatial2DParamsSchema = z3.object({
  positions: z3.array(z3.tuple([gpuNumber, gpuNumber])).min(1).max(PARAM_LIMITS.maxSpatialObjects),
  coordinate_units: units
}).strict();
var CorrelogramPairSchema = z3.object({
  reference_label: displayText(240),
  target_label: displayText(240)
}).strict();
var CorrelogramStatisticSchema = z3.discriminatedUnion("kind", [
  z3.object({ kind: z3.literal("raw_pair_count"), units: z3.literal("count") }).strict(),
  z3.object({ kind: z3.literal("weighted_pair_sum"), units }).strict(),
  z3.object({
    kind: z3.literal("pair_rate_hz"),
    units: z3.literal("Hz"),
    exposure_s: z3.number().positive().max(Number.MAX_VALUE)
  }).strict(),
  z3.object({
    kind: z3.literal("pearson_coefficient"),
    units: z3.literal("1"),
    sample_count: z3.number().int().positive().max(Number.MAX_SAFE_INTEGER)
  }).strict()
]);
...
var CorrelogramParamsSchema = z3.object({
  lags_ms: timeArray.min(1),
  values: gpuArray.min(1),
  bin_width_ms: z3.number().positive().max(Number.MAX_VALUE),
  tau_max_ms: z3.number().positive().max(Number.MAX_VALUE),
  counting_start_ms: z3.number(),
  counting_stop_ms: z3.number(),
  pair: CorrelogramPairSchema,
  lag_convention: z3.literal("positive_target_after_reference"),
  binning: z3.literal("left_closed_right_open"),
  zero_lag_policy: z3.enum(["included", "excluded_self_pairs"]),
...
var StimulusResponseParamsSchema = z3.object({
  times_ms: timeArray.min(1),
...
var AnimationReplayParamsSchema = z3.object({
  frames: z3.array(
    z3.object({
      time_ms: z3.number().nonnegative(),
      state: z3.record(normalizedRecordKey2, z3.unknown()).refine((state) => Object.keys(state).length > 0, {
...
function toPortableJsonSchema(schemaSource) {
  const schema = z4.toJSONSchema(schemaSource, { io: "input" });
...
var HostRendererInvocationSchema = z5.object({
  skill: z5.string().trim().min(1).max(80).regex(SAFE_DISPLAY_STRING_PATTERN, "skill must not contain display control characters"),
  specVersion: z5.literal(CORTEXEL_SPEC_VERSION).optional(),
  ...
  rendererRoute: z5.enum(VALID_RENDERER_ROUTES).optional()
}).strict();
...
//# sourceMappingURL=chunk-FFYJVPAY.js.map