...
function compareExactUnitArraySumToDifference(values, valueUnit, lower, upper) {
	...
	let sumUnits = 0n;
	for (const value of values) sumUnits += finiteBinary64ToMinSubnormalUnits(value);
	const scale = exactUnitScale(unit);
	const sum = {
...
function convertExactUnitSum(terms, targetUnit) {
	...
	try {
		const converted = exactRationalToBinary64(sum.numerator * targetScale.denominator, sum.denominator * targetScale.numerator, sum.binaryExponent - targetScale.binaryExponent);
...
function convert(value, from, to) {
	...
	let converted;
	try {
		converted = exactBinary64MultiplyByRational(value, ratio.numerator, ratio.denominator, ratio.binaryExponent);
	} catch (error) {
...
function conversionFactor(from, to) {
	...
	return exactRationalToBinary64(ratio.numerator, ratio.denominator, ratio.binaryExponent);
}
...
function conversionReceipt(from, to) {
	...
	return {
		from,
		to,
		factor: exactRationalToBinary64(ratio.numerator, ratio.denominator, ratio.binaryExponent),
		exactFactor: {
...
function compositeDerivativeConversionReceipt(stateFrom, stateTo, derivativeFrom, derivativeTo) {
	...
	return {
		...
		factor: exactRationalToBinary64(composite.numerator, composite.denominator, composite.binaryExponent),
		exactFactor: {
...
function convertCompositeDerivative(value, stateFrom, stateTo, derivativeFrom, derivativeTo) {
	...
	let converted;
	try {
		converted = exactBinary64MultiplyByRational(value, composite.numerator, composite.denominator, composite.binaryExponent);
	} catch (error) {
...
function normalizeDerivativeByExactAxisExtent(value, derivativeFrom, derivativeTo, minimum, maximum) {
	...
	let normalized;
	try {
		normalized = exactRationalToBinary64(valueUnits * ratio.numerator, extentUnits * ratio.denominator, ratio.binaryExponent);
	} catch (error) {
...
function convertDifference(lower, upper, from, to) {
	...
	try {
		return exactRationalToBinary64(differenceUnits * ratio.numerator, ratio.denominator, ratio.binaryExponent - 1074);
	} catch (error) {
...
function divideExactIntegerByConvertedDifference(numerator, integerFactor, lower, upper, from, to) {
	...
	let result;
	try {
		result = exactRationalToBinary64(BigInt(numerator) * ratio.denominator, BigInt(integerFactor) * differenceUnits * ratio.numerator, 1074 - ratio.binaryExponent);
	} catch (error) {
...
function deriveExactCountRateInUnit(count, integerFactor, lower, upper, timeUnit, rateUnit) {
	...
	let result;
	try {
		result = exactRationalToBinary64(BigInt(count) * timeToSeconds.denominator * rateToHz.denominator, BigInt(integerFactor) * differenceUnits * timeToSeconds.numerator * rateToHz.numerator, 1074 - timeToSeconds.binaryExponent - rateToHz.binaryExponent);
	} catch (error) {
...
function deriveExactCountRateWithIntegerFactorsInUnit(count, integerFactors, lower, upper, timeUnit, rateUnit) {
	...
	let result;
	try {
		result = exactRationalToBinary64(BigInt(count) * timeToSeconds.denominator * rateToHz.denominator, integerFactor * differenceUnits * timeToSeconds.numerator * rateToHz.numerator, 1074 - timeToSeconds.binaryExponent - rateToHz.binaryExponent);
	} catch (error) {
		if (error instanceof Error && error.message.includes("overflows")) throw new Error("exact count rate overflowed binary64");
		throw error;
	}
...
function deriveExactAggregateCountRateOverIntervalsInUnit(countTotal, integerFactors, intervals, timeUnit, rateUnit) {
	...
	let result;
	try {
		result = exactRationalToBinary64(countTotal * timeToSeconds.denominator * rateToHz.denominator, integerFactor * weightedDifferenceUnits * timeToSeconds.numerator * rateToHz.numerator, 1074 - timeToSeconds.binaryExponent - rateToHz.binaryExponent);
	} catch (error) {
...
function deriveExactCountRateMinusAggregateRateOverIntervalsInUnit(count, integerFactors, lower, upper, aggregateCount, aggregateIntegerFactors, intervals, timeUnit, rateUnit) {
	...
	let result;
	try {
		result = exactRationalToBinary64(signedFractionNumerator * timeToSeconds.denominator * rateToHz.denominator, fractionDenominator * timeToSeconds.numerator * rateToHz.numerator, 1074 - timeToSeconds.binaryExponent - rateToHz.binaryExponent);
	} catch (error) {
...
function deriveExactAggregateCountRateInUnit(countTotal, integerFactor, estimatorDenominator, durationValue, durationUnit, rateUnit) {
	...
	let result;
	try {
		result = exactRationalToBinary64(countTotal * timeToSeconds.denominator * rateToHz.denominator, BigInt(integerFactor) * BigInt(estimatorDenominator) * durationUnits * timeToSeconds.numerator * rateToHz.numerator, 1074 - timeToSeconds.binaryExponent - rateToHz.binaryExponent);
	} catch (error) {
		if (error instanceof Error && error.message.includes("overflows")) throw new Error("exact aggregate count rate overflowed binary64");
		throw error;
	}
...
function materializeWidthBins(start, stop, width) {
	...
	try {
		reconstructedStop = exactRationalToBinary64(startUnits + BigInt(count) * widthUnits, 1n, coefficientExponent);
	} catch {
		return {
			ok: false,
			reason: "unrepresentable"
