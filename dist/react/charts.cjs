...
__export(charts_exports, {
  ...
  formatChartInterval: () => formatChartInterval,
  formatChartNumber: () => formatChartNumber,
  formatDistinctChartNumbers: () => formatDistinctChartNumbers,
  formatExactChartNumber: () => formatExactChartNumber,
...
var KnowledgeGraphEvidenceRefSchema = import_zod2.z.discriminatedUnion("kind", [
  ...
  import_zod2.z.object({
    ...
    page: import_zod2.z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).refine((value) => !Object.is(value, -0), "page must not be negative zero").optional(),
    locator: displayText(KNOWLEDGE_GRAPH_LIMITS.maxLocatorLength).optional(),
    excerpt: displayText(KNOWLEDGE_GRAPH_LIMITS.maxExcerptLength).optional(),
