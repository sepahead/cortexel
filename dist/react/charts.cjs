...
//#region react/charts/chartGeometry.ts
const REFERENCE_CHART_DIMENSIONS = Object.freeze({
	width: 960,
	height: 540,
	minWidth: 320,
	minHeight: 240,
...
function normalizeChartDimension(value, fallback, minimum) {
	...
	return Math.min(REFERENCE_CHART_DIMENSIONS.maxDimension, Math.max(minimum, Math.round(value)));
}
...
function paddedSingleton(value) {
	...
	const padding = Math.abs(value) * .05;
...
function numericDomain(values, options = {}) {
	...
	if (min === max) {
		if (options.includeZero && min === 0) return {
			min: 0,
			max: 1
		};
...
function linePath(xs, ys, xDomain, yDomain, frame) {
	...
	for (let index = 0; index < count; index++) {
		...
		path += `${index === 0 ? "M" : "L"}${svgNumber$1(x)} ${svgNumber$1(y)}`;
	}
	return path;
}
function sortedLinePath(xs, ys, xDomain, yDomain, frame) {
	const pairs = new Array(Math.min(xs.length, ys.length));
	for (let index = 0; index < pairs.length; index++) pairs[index] = {
		x: xs[index],
		y: ys[index],
		order: index
	};
...
function pointPath(xs, ys, xDomain, yDomain, frame, radius = 3) {
	...
	for (let index = 0; index < count; index++) {
		const x = chartX(xs[index], xDomain, frame);
		const y = chartY(ys[index], yDomain, frame);
		path += `M${svgNumber$1(x - radius)} ${svgNumber$1(y)}a${radius} ${radius} 0 1 0 ${svgNumber$1(radius * 2)} 0a${radius} ${radius} 0 1 0 ${svgNumber$1(-radius * 2)} 0`;
	}
	return path;
}
...
function boundedExtremaIndices(values, maximumSamples) {
	...
	if (maximum === 3) {
		let extreme = 1;
		for (let index = 2; index < count - 1; index++) if (Math.abs(values[index]) > Math.abs(values[extreme])) extreme = index;
		return [
			0,
			extreme,
			count - 1
	...
	for (let bucket = 0; bucket < bucketCount; bucket++) {
		...
		} else indices.push(maximumIndex, minimumIndex);
	}
...
function binnedStepPath(centers, values, binWidth, xDomain, yDomain, frame, maximumSamples = 4096) {
	...
	const indices = boundedExtremaIndices(values.slice(0, sourceSampleCount), maximumSamples);
	...
	for (let outputIndex = 0; outputIndex < indices.length; outputIndex++) {
		...
		if (index === previousIndex + 1) path += `V${svgNumber$1(y)}H${svgNumber$1(right)}`;
		else path += `M${svgNumber$1(left)} ${svgNumber$1(y)}H${svgNumber$1(right)}`;
		previousIndex = index;
	}
	return {
		path,
...
function stemPath(xs, ys, xDomain, yDomain, frame) {
	...
	for (let index = 0; index < count; index++) {
		const x = chartX(xs[index], xDomain, frame);
		const y = chartY(ys[index], yDomain, frame);
		path += `M${svgNumber$1(x)} ${svgNumber$1(baseline)}V${svgNumber$1(y)}`;
	}
	return path;
}
...
function boundedStemPointPaths(xs, ys, xDomain, yDomain, frame, maximumSamples = 4096, radius = 2.75) {
	...
	for (let outputIndex = 0; outputIndex < indices.length; outputIndex++) {
		...
		const x = chartX(xs[index], xDomain, frame);
		const y = chartY(ys[index], yDomain, frame);
		stems += `M${svgNumber$1(x)} ${svgNumber$1(baseline)}V${svgNumber$1(y)}`;
		points += `M${svgNumber$1(x - radius)} ${svgNumber$1(y)}a${radius} ${radius} 0 1 0 ${svgNumber$1(radius * 2)} 0a${radius} ${radius} 0 1 0 ${svgNumber$1(-radius * 2)} 0`;
	}
	return {
		stems,
		points,
...
function rasterTickPath(times, senders, xDomain, yDomain, frame, halfHeight = 3) {
	...
	for (let index = 0; index < count; index++) {
		...
		path += `M${svgNumber$1(x)} ${svgNumber$1(y - halfHeight)}V${svgNumber$1(y + halfHeight)}`;
	}
	return path;
}
function histogramBarPath(centers, values, binWidth, xDomain, yDomain, frame) {
	...
	for (let index = 0; index < count; index++) {
		...
		path += `M${svgNumber$1(left)} ${svgNumber$1(baseline)}H${svgNumber$1(right)}V${svgNumber$1(top)}H${svgNumber$1(left)}Z`;
	}
	return path;
}
function phasePlaneSamples(axisOrder, grid, derivatives, derivativeTimeUnit) {
	...
	let outputIndex = 0;
	for (let xIndex = 0; xIndex < xs.length; xIndex++) for (let yIndex = 0; yIndex < ys.length; yIndex++) {
...
function phasePlaneArrowPath(samples, xDomain, yDomain, frame) {
	const maxLength = Math.max(4, Math.min(chartPlotWidth(frame), chartPlotHeight(frame)) / 24);
	let path = "";
	for (let index = 0; index < samples.length; index++) {
		...
		const wing = Math.min(4, length * .3);
		const backX = endX - ux * wing;
		const backY = endY - uy * wing;
		const perpendicularX = -uy * wing * .55;
		const perpendicularY = ux * wing * .55;
		path += `M${svgNumber$1(startX)} ${svgNumber$1(startY)}L${svgNumber$1(endX)} ${svgNumber$1(endY)}M${svgNumber$1(endX)} ${svgNumber$1(endY)}L${svgNumber$1(backX + perpendicularX)} ${svgNumber$1(backY + perpendicularY)}M${svgNumber$1(endX)} ${svgNumber$1(endY)}L${svgNumber$1(backX - perpendicularX)} ${svgNumber$1(backY - perpendicularY)}`;
	}
	return path;
}
...
function formatChartNumber(value) {
	...
	if (magnitude >= 1e4 || magnitude < .001) return value.toExponential(2);
...
//#endregion
//#region react/charts/topologyGeometry.ts
...
function rectanglePath(x, y, width, height) {
	return `M${svgNumber(x)} ${svgNumber(y)}H${svgNumber(x + width)}V${svgNumber(y + height)}H${svgNumber(x)}Z`;
}
const MATRIX_VALUE_LEVELS_PER_SIGN = 8;
/**
...
function matrixValueBucketPaths(cells, sourceCount, targetCount, frame) {
	...
	for (let index = 0; index < cells.length; index++) maximumAbsoluteValue = Math.max(maximumAbsoluteValue, Math.abs(cells[index].value));
	...
	for (let index = 0; index < cells.length; index++) {
		...
		const level = sign === 0 || maximumAbsoluteValue === 0 ? 0 : Math.max(1, Math.min(8, Math.ceil(Math.abs(cell.value) / maximumAbsoluteValue * 8)));
	...
	const buckets = [...paths.entries()].map(([key, value]) => ({
		key,
		...value
...
function quadraticPoint(start, controlX, controlY, end, t) {
	const inverse = 1 - t;
	return [inverse * inverse * start.x + 2 * inverse * t * controlX + t * t * end.x, inverse * inverse * start.y + 2 * inverse * t * controlY + t * t * end.y];
}
function arrowTriangle(tipX, tipY, directionX, directionY, length, width) {
	...
	const px = -uy * width;
	const py = ux * width;
	return `M${svgNumber(tipX)} ${svgNumber(tipY)}L${svgNumber(baseX + px)} ${svgNumber(baseY + py)}L${svgNumber(baseX - px)} ${svgNumber(baseY - py)}Z`;
}
/**
...
function circleTopologyGeometry(nodes, edges, frame, nodeRadius = 4) {
	...
	const layoutRadius = nodes.length <= 1 ? 0 : Math.max(18, Math.min(chartPlotWidth(frame), chartPlotHeight(frame)) * .36);
	...
	for (let index = 0; index < nodes.length; index++) {
		...
		nodePath += `M${svgNumber(position.x - nodeRadius)} ${svgNumber(position.y)}a${svgNumber(nodeRadius)} ${svgNumber(nodeRadius)} 0 1 0 ${svgNumber(nodeRadius * 2)} 0a${svgNumber(nodeRadius)} ${svgNumber(nodeRadius)} 0 1 0 ${svgNumber(-nodeRadius * 2)} 0`;
	}
	...
	for (let index = 0; index < edges.length; index++) {
		...
		if (nodeKey(edge.source) === nodeKey(edge.target)) {
			...
			const startX = source.x + nodeRadius * .65;
			const startY = source.y - nodeRadius * .75;
			const endX = source.x - nodeRadius * .65;
			const endY = source.y - nodeRadius * .75;
			edgePath += `M${svgNumber(startX)} ${svgNumber(startY)}C${svgNumber(source.x + loopRadius)} ${svgNumber(source.y - loopRadius * 1.7)} ${svgNumber(source.x - loopRadius)} ${svgNumber(source.y - loopRadius * 1.7)} ${svgNumber(endX)} ${svgNumber(endY)}`;
			arrowPath += arrowTriangle(endX, endY, -1, .45, Math.max(4, nodeRadius * 1.1), Math.max(2, nodeRadius * .55));
			continue;
		}
		...
		edgePath += `M${svgNumber(start.x)} ${svgNumber(start.y)}Q${svgNumber(controlX)} ${svgNumber(controlY)} ${svgNumber(end.x)} ${svgNumber(end.y)}`;
		const [arrowX, arrowY] = quadraticPoint(start, controlX, controlY, end, .94);
		const tangentX = 2 * (1 - .94) * (controlX - start.x) + 1.88 * (end.x - controlX);
		const tangentY = 2 * (1 - .94) * (controlY - start.y) + 1.88 * (end.y - controlY);
		arrowPath += arrowTriangle(arrowX, arrowY, tangentX, tangentY, Math.max(4, nodeRadius * 1.1), Math.max(2, nodeRadius * .55));
	}
	return {
		nodePath,
		edgePath,
...
function aggregateDegreeBins(degrees, nodeCounts, values, maximumBins = 512) {
	...
	const groupSize = Math.max(1, Math.ceil(count / (Number.isFinite(maximumBins) ? Math.max(1, Math.floor(maximumBins)) : 1)));
	const bins = [];
	...
	for (let start = 0; start < count; start += groupSize) {
		...
		const left = minimumDegree - .5;
		const right = maximumDegree + .5;
		bins.push({
			minimumDegree,
			maximumDegree,
			center: (left + right) / 2,
...
//#endregion
//#region react/charts/ReferenceChartScene.tsx
const REFERENCE_CHART_SKILLS = Object.freeze([
	"nest.voltage_trace",
...
function declaredInput(args, key) {
	...
	if (typeof value === "string") return require_knowledgeGraphLimits.safeDiagnosticText(value, 120);
	if (typeof value === "number" || value === true) return require_knowledgeGraphLimits.safeDiagnosticText(String(value), 120);
}
...
function makeFrame(width, height, requestedRight = 28) {
	...
	return {
		...
		height,
		left,
		right: Math.min(requestedRight, Math.max(18, width - left - MIN_REFERENCE_PLOT_WIDTH)),
		top: 72,
		bottom: 68
	};
}
...
function metadataValue(value, limit = 180) {
	if (typeof value === "string") return require_knowledgeGraphLimits.safeDiagnosticText(value, limit);
	if (typeof value === "number" || typeof value === "boolean") return require_knowledgeGraphLimits.safeDiagnosticText(String(value), limit);
	try {
		return require_knowledgeGraphLimits.safeDiagnosticText(JSON.stringify(value), limit);
	} catch {
		return "<unavailable>";
	}
}
function sampledIndices(length, maximum = 8) {
	...
	for (let index = 0; index < count; index++) indices[index] = Math.round(index * (length - 1) / (count - 1));
	return [...new Set(indices)];
}
function matrixBucketPaint(bucket, palette, skill, neutralForeground) {
	if (bucket.sign === 0) return {
...
function PaginatedChartData({ label, rowCount, rowAt, pageSize = DEFAULT_CHART_DATA_PAGE_SIZE, foreground, background }) {
	...
	for (let index = start; index < stop; index++) rows[index - start] = {
		index,
		text: rowAt(index)
	};
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		...
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				"aria-live": "polite",
				"aria-atomic": "true",
				children: safeRowCount === 0 ? `${label}: no rows.` : `${label}: rows ${start + 1}–${stop} of ${safeRowCount}; page ${page + 1} of ${pageCount}.`
			}),
			rows.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ol", {
				start: start + 1,
				children: rows.map((row) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("bdi", {
					dir: "auto",
					style: { unicodeBidi: "isolate" },
					children: row.text
				}) }, row.index))
			}),
			pageCount > 1 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("nav", {
...
function ChartShell({ id, skill, scene, title, description, metadata, note, accessibleDetails = [], accessibleDetailsLabel = "Series summary", xLabel, yLabel, xDomain, yDomain, frame, colors, legend = [], xTicks: requestedXTicks, yTicks: requestedYTicks, sampleCount, dataRows, children }) {
		...
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
				...
				children: [
					...
					metadata && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("text", {
						x: frame.left,
						y: 51,
						fill: colors.muted,
						fontSize: 11,
					...
					children,
					legendEntries.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("g", {
						"aria-label": "Series legend",
						children: [legendEntries.map((entry, index) => {
						...
						}), legend.length > legendEntries.length && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("text", {
			...
			dataRows && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PaginatedChartData, {
			...
			(note || accessibleDetails.length > 0) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("figcaption", {
				...
				children: [note && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { children: note }), accessibleDetails.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					"aria-label": accessibleDetailsLabel,
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [accessibleDetailsLabel, ": "] }), accessibleDetails.join(" ")]
				})]
			})
		]
	});
...
function sampledUniqueValues(values, maximum = 8) {
	...
	for (let index = 0; index < maximum; index++) sampled[index] = unique[Math.round(index * (unique.length - 1) / (maximum - 1))];
	return sampled;
}
function TraceChart(args, width, height, id) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ChartShell, {
		id,
		skill: args.skill,
		scene: args.scene,
		...
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("g", {
			fill: "none",
			children: params.series.map((series, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
...
const POPULATION_RATE_PATH_SAMPLE_BUDGET = 8192;
...
function PopulationRateChart(args, width, height, id) {
	...
	const yDomain = nestedNumericDomain(params.series.map((series) => series.rates_hz), { includeZero: true });
	...
	const perSeriesBudget = Math.max(2, Math.floor(POPULATION_RATE_PATH_SAMPLE_BUDGET / params.series.length));
	const paths = params.series.map((series) => binnedStepPath(params.bin_centers_ms, series.rates_hz, params.bin_width_ms, xDomain, yDomain, frame, perSeriesBudget));
	const compacted = paths.some((path) => path.compacted);
	const seriesDetails = params.series.map(populationRateSeriesDetail);
	const formula = "rates_hz = spike_counts × 1000 ÷ (recorded_sender_count × bin_width_ms)";
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ChartShell, {
		id,
		skill: args.skill,
		scene: args.scene,
...
function RateResponseChart(args, width, height, id) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ChartShell, {
		id,
		skill: args.skill,
		scene: args.scene,
		title: "F–I response",
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
			"data-mark": "fi-line",
			d: sortedLinePath(params.stimulus_amplitudes, params.rates_hz, xDomain, yDomain, frame),
			fill: "none",
			...
			"data-mark": "fi-points",
			d: pointPath(params.stimulus_amplitudes, params.rates_hz, xDomain, yDomain, frame),
			fill: args.palette.excitatory
		})]
	});
}
function HistogramChart({ args, width, height, id, title, description, metadata, xLabel, yLabel, centers, values, binWidth, color, alignmentLabel }) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ChartShell, {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
			...
			strokeWidth: .75,
			vectorEffect: "non-scaling-stroke"
		}), alignmentLabel && zeroInDomain && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("g", {
			"data-mark": "alignment-zero",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("line", {
				...
				fontSize: 10,
				children: ["t=0: ", alignmentLabel.length > 36 ? `${alignmentLabel.slice(0, 35)}…` : alignmentLabel]
			})]
		})]
	});
}
function IsiChart(args, width, height, id) {
	const params = args.params;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HistogramChart, {
		args,
		width,
		height,
		id,
...
function correlogramStatisticDetail(statistic) {
	switch (statistic.kind) {
		case "pair_rate_hz": return `${statistic.kind} (${statistic.units}), exposure ${formatChartNumber(statistic.exposure_s)} s`;
		case "pearson_coefficient": return `${statistic.kind} (${statistic.units}), ${statistic.sample_count} samples`;
		default: return `${statistic.kind} (${statistic.units})`;
	}
}
function CorrelogramChart(args, width, height, id) {
	...
	const marks = boundedStemPointPaths(params.lags_ms, params.values, xDomain, yDomain, frame);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ChartShell, {
		id,
		skill: args.skill,
		scene: args.scene,
		...
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("g", {
				"data-mark": "zero-lag-reference",
				"data-zero-bin-present": params.lags_ms.includes(0),
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("line", {
					x1: zeroX,
					y1: frame.top,
					x2: zeroX,
...
const WEIGHT_RENDER_BIN_BUDGET = 4096;
function WeightHistogramChart(args, width, height, id) {
	const params = args.params;
	const aggregated = aggregateUniformHistogramBins(params.bin_centers, params.weight_counts, params.values, params.bin_width, params.normalization, WEIGHT_RENDER_BIN_BUDGET);
	const frame = makeFrame(width, height);
	const xDomain = {
		min: params.window_start,
		max: params.window_stop
	};
	const yDomain = numericDomain(aggregated.bins.map((bin) => bin.value), { includeZero: true });
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ChartShell, {
		...
		accessibleDetails: [`Raw connection count is ${aggregated.sourceRawCount} before and ${aggregated.renderedRawCount} after display grouping.`],
...
function PlasticityChart(args, width, height, id) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ChartShell, {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
			...
			strokeWidth: 2,
			vectorEffect: "non-scaling-stroke"
		}), params.weights.length === 1 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
			"data-mark": "weight-points",
			"data-point-count": 1,
			d: pointPath(params.times_ms, params.weights, xDomain, yDomain, frame, 3),
...
function PhasePlaneChart(args, width, height, id) {
	...
	const frame = makeFrame(width, height);
	const samples = phasePlaneSamples(params.axis_order, params.grid, params.derivatives, params.derivative_time_unit);
	const zeroSamples = samples.filter((sample) => sample.dx === 0 && sample.dy === 0);
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ChartShell, {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
			...
			vectorEffect: "non-scaling-stroke"
		}), zeroSamples.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
			"data-mark": "phase-zero-vectors",
			"data-zero-vector-count": zeroSamples.length,
			d: pointPath(zeroSamples.map((sample) => sample.x), zeroSamples.map((sample) => sample.y), xDomain, yDomain, frame, 3),
			fill: "none",
			stroke: args.palette.orange,
			strokeWidth: 1.4,
			vectorEffect: "non-scaling-stroke"
...
function MatrixChart(args, width, height, id) {
	...
	for (let index = 0; index < params.source_ids.length; index++) sourceIndex.set(params.source_ids[index], index);
	for (let index = 0; index < params.target_ids.length; index++) targetIndex.set(params.target_ids[index], index);
	...
	const geometry = matrixValueBucketPaths(cells, params.source_ids.length, params.target_ids.length, frame);
	const cellStrokeWidth = Math.min(chartPlotWidth(frame) / params.source_ids.length, chartPlotHeight(frame) / params.target_ids.length) >= 1.5 ? .35 : 0;
	...
	const legendEntries = skill === "nest.adjacency_matrix" ? [[
		...
		[
			"cool hue: negative numeric weight",
			args.palette.cyan,
			1
		],
		[
			"warm hue: positive numeric weight",
			args.palette.orange,
			1
		]
	] : [[
		...
		"teal hue: positive numeric delay",
		args.palette.teal,
		1
	]];
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ChartShell, {
		...
		xDomain: {
			min: 0,
			max: Math.max(1, params.source_ids.length)
		},
		yDomain: {
			min: 0,
			max: Math.max(1, params.target_ids.length)
		},
		xTicks: [],
		yTicks: [],
		frame,
		...
		dataRows: {
			...
			rowAt: (index) => {
				if (index < params.source_ids.length) return `Source-axis column ${index + 1} of ${params.source_ids.length} (declared order): node ID ${params.source_ids[index]}.`;
				const targetRow = index - params.source_ids.length;
				if (targetRow < params.target_ids.length) return `Target-axis row ${targetRow + 1} of ${params.target_ids.length} (declared order): node ID ${params.target_ids[targetRow]}.`;
				...
				if (skill === "nest.adjacency_matrix") return `Present-cell record ${cellIndex + 1} of ${params.cells.length}: target node ID ${cell.target_id} at declared row ${declaredRow}, source node ID ${cell.source_id} at declared column ${declaredColumn}; ${cell.connection_count} connection${cell.connection_count === 1 ? "" : "s"}; binary presence.`;
				const measured = cell;
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("g", {
				"aria-hidden": "true",
				"data-mark": "matrix-axis-identities",
				children: [sourceTicks.map((axisIndex) => {
					const x = frame.left + (axisIndex + .5) / params.source_ids.length * chartPlotWidth(frame);
					...
					const y = frame.top + (axisIndex + .5) / params.target_ids.length * chartPlotHeight(frame);
					return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("text", {
						x: frame.left - 9,
						y: y + 3,
						fill: colors.muted,
			...
			showLegend && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("g", {
				"aria-label": "Matrix value legend",
				"data-mark": "matrix-value-legend",
				children: legendEntries.map(([label, color, opacity], legendIndex) => {
...
function ConnectionGraphChart(args, width, height, id) {
	...
	const isolateCount = params.nodes.reduce((count, node) => count + (endpointIds.has(node.id) ? 0 : 1), 0);
	const scope = metadataValue(params.snapshot_scope);
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ChartShell, {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("g", {
				"aria-hidden": "true",
				"data-mark": "connection-node-labels",
				children: labels.map((nodeIndex) => {
...
function DegreeDistributionChart(args, width, height, id) {
	const params = args.params;
	const aggregated = aggregateDegreeBins(params.degrees, params.node_counts, params.values, DEGREE_RENDER_BIN_BUDGET);
	const frame = makeFrame(width, height);
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ChartShell, {
		...
		accessibleDetails: [`Raw node-count mass: ${formatChartNumber(aggregated.sourceNodeMass)} before and ${formatChartNumber(aggregated.renderedNodeMass)} after display grouping.`, `Displayed value mass: ${formatChartNumber(aggregated.sourceValueMass)} before and ${formatChartNumber(aggregated.renderedValueMass)} after grouping.`],
...
const DELAY_RENDER_BIN_BUDGET = 4096;
function DelayDistributionChart(args, width, height, id) {
	const params = args.params;
	const aggregated = aggregateUniformHistogramBins(params.bin_centers_ms, params.delay_counts, params.values, params.bin_width_ms, params.normalization, DELAY_RENDER_BIN_BUDGET);
	const frame = makeFrame(width, height);
	const xDomain = {
		min: params.window_start_ms,
		max: params.window_stop_ms
	};
	const yDomain = numericDomain(aggregated.bins.map((bin) => bin.value), { includeZero: true });
	const path = variableHistogramPath(aggregated.bins, xDomain, yDomain, frame);
	const scope = metadataValue(params.snapshot_scope);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ChartShell, {
		...
		accessibleDetails: [`Raw delay-event count is ${aggregated.sourceRawCount} before and ${aggregated.renderedRawCount} after display grouping.`],
...
function SpatialMap2DChart(args, width, height, id) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ChartShell, {
		...
		accessibleDetails: [`${params.nodes.length} source positions and ${params.nodes.length} rendered positions; none omitted.`, `Node id range in declared order: ${params.nodes[0]?.id ?? "none"}…${params.nodes[params.nodes.length - 1]?.id ?? "none"}.`],
...
function UnsupportedReferenceChart({ skill, scene }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		role: "alert",
		className: "cortexel-reference-chart-unsupported",
		children: [
...
function validateReferenceChartInput(args) {
	const contract = require_authoring.getSkill(args.skill);
	if (!contract || contract.scene !== args.scene) return {
		ok: false,
		messages: ["The skill and scene do not identify the same registered Cortexel chart contract."]
	};
	const checked = require_authoring.validateSkillParams(args.skill, args.params);
	if (!checked.ok) return {
		ok: false,
		messages: checked.errors.map((error) => require_knowledgeGraphLimits.safeDiagnosticText(`${error.path}: ${error.message}`, 500))
	};
	return {
		ok: true,
...
function InvalidReferenceChartInput({ messages }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		role: "alert",
		className: "cortexel-reference-chart-invalid",
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: "Invalid reference chart input" }),
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: "The low-level chart surface validates only its registered skill, scene, and params. Validate the complete spec with ReferenceVizSpecFigure to bind provenance and the honesty caption." }),
...
function ReferenceChartScene(args) {
	const checked = (0, react.useMemo)(() => validateReferenceChartInput(args), [
		args.params,
		args.scene,
		args.skill
	]);
	if (!checked.ok) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(InvalidReferenceChartInput, { messages: checked.messages });
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CheckedReferenceChartScene, {
		...args,
		params: checked.params
	});
}
...
function CheckedReferenceChartScene(args) {
	const id = `cortexel-chart-${(0, react.useId)().replace(/[^a-zA-Z0-9_-]/g, "")}`;
	const width = normalizeChartDimension(args.width, REFERENCE_CHART_DIMENSIONS.width, REFERENCE_CHART_DIMENSIONS.minWidth);
	const height = normalizeChartDimension(args.height, REFERENCE_CHART_DIMENSIONS.height, REFERENCE_CHART_DIMENSIONS.minHeight);
	switch (args.skill) {
		case "nest.voltage_trace": return TraceChart(args, width, height, id);
		case "nest.astrocyte_dynamics": return AstrocyteChart(args, width, height, id);
		case "nest.spike_raster": return SpikeRasterChart(args, width, height, id);
		case "nest.population_rate": return PopulationRateChart(args, width, height, id);
		case "nest.rate_response": return RateResponseChart(args, width, height, id);
		case "nest.isi_distribution": return IsiChart(args, width, height, id);
		case "nest.psth": return PsthChart(args, width, height, id);
		case "nest.correlogram": return CorrelogramChart(args, width, height, id);
		case "nest.weight_histogram": return WeightHistogramChart(args, width, height, id);
		case "nest.plasticity_dynamics": return PlasticityChart(args, width, height, id);
		case "nest.phase_plane": return PhasePlaneChart(args, width, height, id);
		case "nest.connection_graph": return ConnectionGraphChart(args, width, height, id);
		case "nest.adjacency_matrix":
		case "nest.weight_matrix":
		case "nest.delay_matrix": return MatrixChart(args, width, height, id);
		case "nest.in_degree_distribution":
		case "nest.out_degree_distribution": return DegreeDistributionChart(args, width, height, id);
		case "nest.delay_distribution": return DelayDistributionChart(args, width, height, id);
		case "nest.spatial_map_2d": return SpatialMap2DChart(args, width, height, id);
		default: return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(UnsupportedReferenceChart, {
			skill: args.skill,
			scene: args.scene
		});
	}
...
//#endregion
//#region react/charts/ReferenceVizSpecFigure.tsx
...
function ReferenceVizSpecFigure({ spec, skillId, active = true, activePalette, width, height, onError, onInvocationError }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_VizSpecRenderer.VizSpecRenderer, {
		spec,
		skillId,
		active,
		activePalette,
		...
		renderScene: (args) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CheckedReferenceChartScene, {
			...args,
			width,
			height
		})
...
//#endregion
exports.MATRIX_VALUE_LEVELS_PER_SIGN = MATRIX_VALUE_LEVELS_PER_SIGN;
exports.REFERENCE_CHART_DIMENSIONS = REFERENCE_CHART_DIMENSIONS;
exports.REFERENCE_CHART_SKILLS = REFERENCE_CHART_SKILLS;
exports.ReferenceChartScene = ReferenceChartScene;
exports.ReferenceVizSpecFigure = ReferenceVizSpecFigure;
exports.aggregateDegreeBins = aggregateDegreeBins;
exports.aggregateUniformHistogramBins = aggregateUniformHistogramBins;
exports.binnedStepPath = binnedStepPath;
exports.boundedExtremaIndices = boundedExtremaIndices;
exports.boundedStemPointPaths = boundedStemPointPaths;
exports.chartPlotHeight = chartPlotHeight;
exports.chartPlotWidth = chartPlotWidth;
exports.chartX = chartX;
exports.chartY = chartY;
exports.circleTopologyGeometry = circleTopologyGeometry;
exports.equalAspectDomains = equalAspectDomains;
exports.formatChartInterval = formatChartInterval;
exports.formatChartNumber = formatChartNumber;
exports.formatDistinctChartNumbers = formatDistinctChartNumbers;
exports.formatExactChartNumber = formatExactChartNumber;
exports.histogramBarPath = histogramBarPath;
exports.histogramDomain = histogramDomain;
exports.linePath = linePath;
exports.matrixValueBucketPaths = matrixValueBucketPaths;
exports.nestedNumericDomain = nestedNumericDomain;
exports.normalizeChartDimension = normalizeChartDimension;
exports.numericDomain = numericDomain;
exports.phasePlaneArrowPath = phasePlaneArrowPath;
exports.phasePlaneSamples = phasePlaneSamples;
exports.pointPath = pointPath;
exports.rasterTickPath = rasterTickPath;
exports.scaleToRange = scaleToRange;
exports.sortedLinePath = sortedLinePath;
exports.stemPath = stemPath;
exports.tickValues = tickValues;
exports.variableHistogramPath = variableHistogramPath;
