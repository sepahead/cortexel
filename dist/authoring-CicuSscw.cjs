...
function turbo(t) {
	...
	const x4 = x3 * x;
	const x5 = x4 * x;
	const r = .13572138 + 4.6153926 * x - 42.66032258 * x2 + 132.13108234 * x3 - 152.94239396 * x4 + 59.28637943 * x5;
	const g = .09140261 + 2.19418839 * x + 4.84296658 * x2 - 14.18503333 * x3 + 4.27729857 * x4 + 2.82956604 * x5;
	const b = .1066733 + 12.64194608 * x - 60.58204836 * x2 + 110.36276771 * x3 - 89.90310912 * x4 + 27.34824973 * x5;
	return [
...
function sampleColormap(name, t) {
	if (name !== "turbo" && !Object.hasOwn(STOP_RGB, name)) throw new RangeError(`unknown colormap '${String(name)}'`);
...
function colormapRgba(name, t, alpha = 1) {
	if (!Number.isFinite(alpha) || alpha < 0 || alpha > 1) throw new RangeError("alpha must be a finite number in [0, 1]");
	const [r, g, b] = sampleColormap(name, t);
...
function colormapGradient(name, angle = 90, stops = 12) {
	...
	if (!Number.isSafeInteger(stops) || stops < 2 || stops > 256) throw new RangeError("gradient stops must be an integer in [2, 256]");
	const parts = [];
...
function colormapSvgStops(name, stops = 8) {
	if (!Number.isSafeInteger(stops) || stops < 2 || stops > 256) throw new RangeError("SVG stops must be an integer in [2, 256]");
	let out = "";
	for (let i = 0; i < stops; i++) {
		const t = i / (stops - 1);
...
const PALETTE_REGISTRY_POLICY = Object.freeze({
	version: "1",
...
const _paletteRegistry = /* @__PURE__ */ new Map();
/** Default palette — Crameri scientific colour maps (batlow + vik). */
const CORTEXEL_PALETTE = {
	voidNavy: "#030711",
	deepNavy: "#050816",
	panel: "#0b1220",
	grid: "#1e293b",
...
const SEMANTIC_PALETTE_KEYS = Object.freeze(Object.keys(CORTEXEL_PALETTE));
const HEX_RE = /^#[0-9a-fA-F]{6}$/;
function snapshotPalette(p) {
	if (p === null || typeof p !== "object" || Array.isArray(p)) throw new TypeError("palette must be an object");
	const prototype = Object.getPrototypeOf(p);
	if (prototype !== Object.prototype && prototype !== null) throw new TypeError("palette must be a plain object");
	const ownKeys = Reflect.ownKeys(p);
	if (ownKeys.some((key) => typeof key === "symbol")) throw new Error("Palette may not contain symbol keys");
	const keys = ownKeys;
	const missing = SEMANTIC_PALETTE_KEYS.filter((key) => !keys.includes(key));
	const extra = keys.filter((key) => !SEMANTIC_PALETTE_KEYS.includes(key));
	...
	for (const key of SEMANTIC_PALETTE_KEYS) {
		...
		if (!descriptor || !("value" in descriptor) || !descriptor.enumerable) throw new Error(`Palette color '${key}' must be an enumerable data property`);
		const val = descriptor.value;
		if (typeof val !== "string") throw new Error(`Palette color '${key}' must be a string`);
		if (!HEX_RE.test(val)) throw new Error(`Palette color '${key}' is not a valid #rrggbb hex: '${val}'`);
		snapshot[key] = val;
	}
	if (snapshot.excitatory.toLowerCase() === snapshot.inhibitory.toLowerCase()) throw new Error("Palette excitatory and inhibitory colors must differ");
	if (snapshot.ltp.toLowerCase() === snapshot.ltd.toLowerCase()) throw new Error("Palette ltp and ltd colors must differ");
	return snapshot;
}
function snapshotPaletteMetadata(metadata) {
	if (metadata === null || typeof metadata !== "object" || Array.isArray(metadata)) throw new Error("palette metadata must be an object");
	const prototype = Object.getPrototypeOf(metadata);
	const keys = Reflect.ownKeys(metadata);
	if (prototype !== Object.prototype && prototype !== null || keys.length !== 3 || keys.some((key) => typeof key !== "string" || ![
		"label",
		"source",
		"diverging"
	].includes(key))) throw new Error("palette metadata must be a strict plain {label,source,diverging} object");
	const values = {};
	for (const key of keys) {
		const descriptor = Object.getOwnPropertyDescriptor(metadata, key);
		if (!descriptor || !("value" in descriptor) || !descriptor.enumerable) throw new Error(`palette metadata '${key}' must be an enumerable data property`);
		values[key] = descriptor.value;
	}
	if (typeof values.label !== "string" || values.label.trim().length === 0 || values.label.length > 120 || typeof values.source !== "string" || values.source.trim().length === 0 || values.source.length > 500 || typeof values.diverging !== "boolean") throw new Error("palette metadata requires bounded label/source strings and diverging boolean");
	return {
...
function registerPalette(name, palette, metadata) {
	if (typeof name !== "string") throw new TypeError("palette name must be a string");
	const normalizedName = name.trim();
	if (normalizedName.length === 0 || normalizedName.length > 60) throw new Error("palette name must contain 1–60 non-whitespace characters");
...
function getPalette(name = "crameri") {
	...
	if (name && name !== "crameri" && !isProduction) try {
		if (typeof console !== "undefined" && console.warn) console.warn(`[cortexel] getPalette('${name}'): not registered, falling back to 'crameri'. Call registerPalette('${name}', ...) at app startup. Available: ${listPalettes().map((p) => p.name).join(", ")}`);
	} catch {}
...
const CORTICAL_LAYER_COLORS = {
	L1: colormapHex("batlow", .05),
	"L2/3": colormapHex("batlow", .28),
	L4: colormapHex("batlow", .48),
	L5: colormapHex("batlow", .68),
	L6: colormapHex("batlow", .9)
};
...
function categorical(i) {
	...
	return CATEGORICAL[(Math.trunc(i) % CATEGORICAL.length + CATEGORICAL.length) % CATEGORICAL.length];
}
const OKABE_ITO = {
	black: "#000000",
	orange: "#e69f00",
...
const CORTEXEL_SPEC_VERSION = "1.5.0";
...
const CORTEXEL_JSON_LIMITS = Object.freeze({
	maxDepth: 32,
	maxNodes: 5e5,
	maxObjectKeys: 1e4,
	maxStringLength: 1e5,
...
const CORTEXEL_JSON_POLICY = Object.freeze({
...
const STRING_NORMALIZATION_POLICY = Object.freeze({
	version: "1",
	...
	displayStringPattern: require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN.source,
	displayStringControls: "reject C0/C1, bidi, zero-width, and BOM controls"
});
const NUMERIC_MODEL_POLICY = Object.freeze({
	version: "1",
...
const JSON_BUDGET_SEMANTICS = Object.freeze({
	version: "1",
...
const JSON_PARAMS_PORTABLE_SCHEMA = Object.freeze({
	type: "object",
...
const DECLARED_INPUTS_PORTABLE_SCHEMA = Object.freeze({
	type: "object",
	maxProperties: 64,
	propertyNames: Object.freeze({
		type: "string",
		...
		allOf: Object.freeze([Object.freeze({ pattern: "^\\S(?:[\\s\\S]*\\S)?$" }), Object.freeze({ pattern: require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN.source })])
	}),
	additionalProperties: Object.freeze({ anyOf: Object.freeze([
		Object.freeze({
			type: "string",
			maxLength: 5e3,
			"x-cortexel-max-utf16-code-units": 5e3,
			pattern: require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN.source
		}),
...
const ENVELOPE_NORMALIZATION_POLICY = Object.freeze({
	version: "1",
...
const normalizedRecordKey$1 = zod.z.string().min(1).max(80).regex(/^\S(?:[\s\S]*\S)?$/, "record keys must already be trimmed and contain a non-whitespace character").regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN, "record keys must not contain display control characters");
/**
...
function cloneExactJson(root) {
	...
	function inspectString(value, path) {
		if (value.length > CORTEXEL_JSON_LIMITS.maxStringLength) return {
			path,
			message: `JSON string exceeds ${CORTEXEL_JSON_LIMITS.maxStringLength} characters`
		};
		totalStringLength += value.length;
		if (totalStringLength > CORTEXEL_JSON_LIMITS.maxTotalStringLength) return {
			path,
		...
		for (let index = 0; index < value.length; index++) {
			...
			if (codeUnit >= 55296 && codeUnit <= 56319) {
				...
				if (!(next >= 56320 && next <= 57343)) return {
					path,
					message: "strings must not contain an unpaired high surrogate"
				};
				index += 1;
			} else if (codeUnit >= 56320 && codeUnit <= 57343) return {
				path,
				message: "strings must not contain an unpaired low surrogate"
			};
		}
		return null;
	}
	function visit(value, path, depth) {
		visited += 1;
		if (visited > CORTEXEL_JSON_LIMITS.maxNodes) return fail(path, `JSON value exceeds ${CORTEXEL_JSON_LIMITS.maxNodes} nodes`);
		if (depth > CORTEXEL_JSON_LIMITS.maxDepth) return fail(path, `JSON nesting exceeds ${CORTEXEL_JSON_LIMITS.maxDepth} levels`);
		...
		if (typeof value === "number") {
			if (!Number.isFinite(value)) return fail(path, "JSON numbers must be finite (NaN/Infinity are not allowed)");
			return Object.is(value, -0) ? fail(path, "negative zero is not stable through JSON.stringify") : {
				ok: true,
				value
			};
		}
		if (typeof value !== "object") return fail(path, `value of type '${typeof value}' is not JSON-serializable`);
		const object = value;
		if (ancestors.has(object)) return fail(path, "circular JSON reference");
		ancestors.add(object);
		try {
			const isRawJson = JSON.isRawJSON;
			if (isRawJson?.(value)) return fail(path, "JSON.rawJSON values are not literal objects and are not allowed");
			if (Array.isArray(value)) {
				const lengthDescriptor = Object.getOwnPropertyDescriptor(value, "length");
				if (!lengthDescriptor || !("value" in lengthDescriptor) || !Number.isSafeInteger(lengthDescriptor.value) || lengthDescriptor.value < 0) return fail(path, "JSON arrays must have an ordinary non-negative length");
				const length = lengthDescriptor.value;
				if (length > CORTEXEL_JSON_LIMITS.maxNodes - visited) return fail(path, `JSON value exceeds ${CORTEXEL_JSON_LIMITS.maxNodes} nodes`);
				const ownKeys = Reflect.ownKeys(value);
				for (const key of ownKeys) {
					if (key === "length") continue;
					if (typeof key !== "string" || !/^(0|[1-9]\d*)$/.test(key) || Number(key) >= length) return fail(path, "JSON arrays may not carry symbol, named, or out-of-range properties");
				}
				const clone = new Array(length);
				for (let i = 0; i < length; i++) {
					const descriptor = Object.getOwnPropertyDescriptor(value, String(i));
					if (!descriptor) return fail([...path, i], "sparse arrays are not allowed in exact JSON");
					if (!("value" in descriptor) || !descriptor.enumerable) return fail([...path, i], "JSON array entries must be enumerable data properties, not accessors");
			...
			if (prototype !== Object.prototype && prototype !== null) return fail(path, "exact JSON must contain plain objects, not class instances");
			const ownKeys = Reflect.ownKeys(value);
			if (ownKeys.some((key) => typeof key === "symbol")) return fail(path, "JSON objects may not contain symbol keys");
			const keys = ownKeys;
			if (keys.length > CORTEXEL_JSON_LIMITS.maxObjectKeys) return fail(path, `JSON object exceeds ${CORTEXEL_JSON_LIMITS.maxObjectKeys} keys`);
			const clone = {};
			for (const key of keys) {
				if (key === "__proto__") return fail([...path, key], "the '__proto__' key is not preserved by the runtime schema parser");
				...
				if (!descriptor || !("value" in descriptor) || !descriptor.enumerable) return fail([...path, key], "JSON object fields must be enumerable data properties, not accessors");
...
const JsonParamsSchema = zod.z.unknown().transform((params, ctx) => {
...
const ProvenanceSchema = zod.z.object({
	source: zod.z.string().trim().min(1).max(200).regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN),
	calibrated_posterior: zod.z.literal(false).default(false),
	advisory_only: zod.z.boolean().default(true),
	is_paper_local_evidence: zod.z.boolean().default(false),
	caption: zod.z.string().trim().max(500).regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN).optional(),
	...
	*  producer's responsibility. */
	declared_inputs: JsonParamsSchema.pipe(zod.z.record(normalizedRecordKey$1, zod.z.union([
		zod.z.string().max(5e3).regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN),
		zod.z.number(),
		zod.z.literal(true)
	]))).refine((inputs) => Object.keys(inputs).length <= 64, { message: "declared_inputs may contain at most 64 keys" }).optional(),
...
const VizSpecSchema = zod.z.object({
	...
	*  cannot recover the skill — this field closes that gap. */
	skill: zod.z.string().trim().min(1).max(80).regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN, "skill must not contain display control characters").optional(),
	/** Optional contract version this spec targets (see CORTEXEL_SPEC_VERSION). */
	specVersion: zod.z.literal(CORTEXEL_SPEC_VERSION).optional(),
	...
	*  a dev-mode warning). When absent, the host's active palette is used. */
	palette: zod.z.string().trim().min(1).max(60).regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN, "palette must not contain display control characters").optional(),
...
function validateVizSpec(input) {
	try {
		...
		if (!exact.success) return {
			ok: false,
			errors: require_knowledgeGraphLimits.formatValidationIssues(exact.error.issues)
		};
		...
		return {
			ok: false,
			errors: require_knowledgeGraphLimits.formatValidationIssues(result.error.issues)
		};
	} catch (error) {
		return {
			ok: false,
			errors: [`(root): validation could not safely inspect the payload: ${require_knowledgeGraphLimits.safeErrorMessage(error)}`]
		};
	}
}

//#endregion
//#region core/provenance.ts
const CONSERVATIVE_PROVENANCE = Object.freeze({
	calibrated_posterior: false,
	advisory_only: true,
	is_paper_local_evidence: false,
	synthetic: false
...
const HONESTY_POLICY = Object.freeze({
	version: "3",
...
function mandatoryDisclosure(p) {
	if (p.synthetic) return HONESTY_POLICY.templates.synthetic;
	if (p.advisory_only) return HONESTY_POLICY.templates.advisory_only;
	if (!p.is_paper_local_evidence) return HONESTY_POLICY.templates.not_paper_local;
...
function composeHonestyCaption(p, contractDisclosures = {}) {
	const parts = [];
	if (contractDisclosures.weakSkill) parts.push(contractDisclosures.weakSkill);
	if (contractDisclosures.externalProvenance) parts.push(contractDisclosures.externalProvenance);
	if (requiresHonestyCaption(p)) parts.push(mandatoryDisclosure(p));
	const note = p.caption?.trim();
	if (note) parts.push(`Caller note (unverified): ${require_knowledgeGraphLimits.safeDiagnosticText(note, 500)}`);
	return parts.length > 0 ? parts.join(" ") : null;
}

//#endregion
//#region core/skills/skillIds.ts
const NEST_SKILL_IDS = Object.freeze([
	"nest.voltage_trace",
	"nest.spike_raster",
...
const SKILL_IDS = NEST_SKILL_IDS;
...
const VIZ_ROUTER_ID = "nest.viz_router";
const NEST_DEVICE_FAMILIES = Object.freeze([
	"multimeter",
	"spike_recorder",
...
const isNestSkillId = isSkillId;
const VALID_RENDERER_ROUTES = Object.freeze([
	"media.trace_figure",
	"media.model_graph",
	"media.webgl_scene",
...
//#endregion
//#region core/skills/provenanceKeys.ts
...
const ProvenanceKeyEnum = zod.z.enum(PROVENANCE_KEYS);
const STRICT_PROVENANCE_POLICY = Object.freeze({
...
const PROVENANCE_KEY_LABELS = Object.freeze({
	device_id: "device id",
...
const PROVENANCE_PARAM_CONSTRAINT_LANGUAGE = Object.freeze({
	version: "4",
	evaluationOrder: Object.freeze([
...
const PROVENANCE_VALUE_CONSTRAINTS = (() => {
	const constraints = Object.create(null);
	for (const key of PROVENANCE_KEYS) constraints[key] = {
...
function declaredProvenanceValueError(key, value) {
	...
	switch (constraint.kind) {
		case "positive_finite_number": return typeof value === "number" && Number.isFinite(value) && value > 0 ? null : `${key} must be a positive finite number`;
		case "nonnegative_finite_number": return typeof value === "number" && Number.isFinite(value) && value >= 0 && !Object.is(value, -0) ? null : `${key} must be a non-negative finite number`;
		case "literal_true": return value === true ? null : "identity_advisory must be literal true (model identity is advisory)";
		case "nonnegative_safe_integer_or_nonblank_string":
			if (typeof value === "number") return Number.isSafeInteger(value) && value >= 0 && !Object.is(value, -0) ? null : `${key} numeric ids must be non-negative safe integers`;
		...
		case "canonical_id_collection": {
			if (typeof value !== "string") return `${key} must be a canonical id-array or digest string`;
			if (constraint.allowDigest && isCanonicalDigest(value)) return null;
			if (constraint.allowOpaqueDigestCount && opaqueDigestCollectionCount(value) !== void 0) return null;
		...
		case "canonical_positive_finite_number_array": {
			...
			if (!constraint.allowedLengths.includes(parsed.values.length)) return `${key} must contain ${constraint.allowedLengths.join(" or ")} elements`;
		...
		case "string": return typeof value === "string" ? null : `${key} must be a string`;
		case "nonblank_string": return typeof value === "string" && value.trim().length > 0 ? null : `${key} must be a non-empty string`;
	}
}
...
function resolveSafeParamPath(params, paramPath) {
	...
	if (segments.length === 0 || segments.some((segment) => !/^[A-Za-z_][A-Za-z0-9_]*$/.test(segment) || segment === "__proto__" || segment === "prototype" || segment === "constructor")) return {
		ok: false,
		message: `params.${paramPath} is not a safe parameter path`
	};
	let value = params;
	for (const segment of segments) {
		if (value === null || typeof value !== "object" || Array.isArray(value) || !Object.hasOwn(value, segment)) return {
			ok: false,
...
function regularTimeAxisError(constraint, actual, params) {
	if (typeof actual !== "number" || !Number.isFinite(actual) || actual <= 0) return `${constraint.provenanceKey} must be a positive finite number`;
	const resolved = resolveSafeParamPath(params, constraint.paramPath);
	if (!resolved.ok) return `cannot verify ${constraint.provenanceKey}: ${resolved.message}`;
	if (!Array.isArray(resolved.value) || !resolved.value.every((value) => typeof value === "number" && Number.isFinite(value))) return `cannot verify ${constraint.provenanceKey}: params.${constraint.paramPath} is not a finite numeric array`;
	if (!Number.isFinite(constraint.absoluteTolerance) || constraint.absoluteTolerance < 0 || !Number.isFinite(constraint.relativeTolerance) || constraint.relativeTolerance < 0 || !Number.isFinite(constraint.roundoffUlps) || constraint.roundoffUlps < 0 || !Number.isFinite(constraint.maxRoundoffFraction) || constraint.maxRoundoffFraction < 0) return `cannot verify ${constraint.provenanceKey}: the contract has invalid sampling tolerances`;
	const times = resolved.value;
	if (times.length < 2) return `cannot verify ${constraint.provenanceKey}: params.${constraint.paramPath} must contain at least two timestamps`;
	for (let index = 1; index < times.length; index++) {
		const left = times[index - 1];
		const right = times[index];
		const delta = right - left;
		if (!(delta > 0) || !Number.isFinite(delta)) return `${constraint.provenanceKey} cannot match params.${constraint.paramPath}: timestamps must be strictly increasing at index ${index}`;
		...
		const boundedRoundoff = candidateRoundoff <= constraint.maxRoundoffFraction * Math.abs(actual) ? candidateRoundoff : 0;
		const tolerance = constraint.absoluteTolerance + constraint.relativeTolerance * relativeScale + boundedRoundoff;
		if (Math.abs(delta - actual) > tolerance) return `${constraint.provenanceKey} (${JSON.stringify(actual)}) must match every adjacent delta in params.${constraint.paramPath}; index ${index} has delta ${JSON.stringify(delta)}`;
	}
	return null;
}
function seriesLabelBindingError(constraint, actual, params) {
	if (typeof actual !== "string" || actual.length === 0) return `${constraint.provenanceKey} must be a nonblank string`;
	if (constraint.separator.length === 0) return `cannot verify ${constraint.provenanceKey}: the contract label separator is empty`;
	const resolved = resolveSafeParamPath(params, constraint.paramPath);
	if (!resolved.ok) return `cannot verify ${constraint.provenanceKey}: ${resolved.message}`;
	if (!Array.isArray(resolved.value) || !resolved.value.every((value) => typeof value === "string")) return `cannot verify ${constraint.provenanceKey}: params.${constraint.paramPath} is not a string array`;
	...
	for (let index = 0; index < resolved.value.length; index++) {
		...
		if (label.endsWith(suffix) && label.slice(0, -suffix.length).trim().length > 0) continue;
...
function parseCanonicalScalarArray(actual, provenanceKey) {
	...
	if (!Array.isArray(parsed) || !parsed.every((value) => value === null || typeof value === "string" || typeof value === "boolean" || typeof value === "number" && Number.isFinite(value) && !Object.is(value, -0))) return {
		ok: false,
		message: `${provenanceKey} must be a canonical JSON array of finite scalar values`
	};
	let canonical;
	try {
		canonical = require_canonicalize.canonicalize(parsed);
	} catch {
		return {
			ok: false,
...
function jsonScalarKey(value) {
	if (value !== null && typeof value !== "string" && typeof value !== "boolean" && !(typeof value === "number" && Number.isFinite(value) && !Object.is(value, -0))) return null;
	try {
...
function parseCanonicalIdArray(actual, provenanceKey, idDomain) {
	...
	if (!parsed.values.every((value) => matchesIdDomain(value, idDomain))) return {
		ok: false,
...
function projectedValues(params, paramPath, field) {
	...
	if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(field) || field === "__proto__" || field === "prototype" || field === "constructor") return {
		ok: false,
		message: `field ${field} is not a safe own-property name`
	};
	const values = [];
	for (let index = 0; index < resolved.value.length; index++) {
		const item = resolved.value[index];
		if (item === null || typeof item !== "object" || Array.isArray(item) || !Object.hasOwn(item, field)) return {
			ok: false,
...
function canonicalJsonParamError(constraint, actual, params) {
	const resolved = resolveSafeParamPath(params, constraint.paramPath);
	if (!resolved.ok) return `cannot verify ${constraint.provenanceKey}: ${resolved.message}`;
	if (!Array.isArray(resolved.value)) return `cannot verify ${constraint.provenanceKey}: params.${constraint.paramPath} is not an array`;
	try {
		if (constraint.allowDigest && isCanonicalDigest(actual) && actual === require_canonicalize.canonicalDigest(resolved.value)) return null;
		const parsed = parseCanonicalScalarArray(actual, constraint.provenanceKey);
		if (!parsed.ok) return parsed.message;
		return require_canonicalize.canonicalize(parsed.values) === require_canonicalize.canonicalize(resolved.value) ? null : `${constraint.provenanceKey} must match params.${constraint.paramPath}`;
	} catch {
...
function projectedCollectionError(constraint, actual, params) {
	const projected = projectedValues(params, constraint.paramPath, constraint.field);
	if (!projected.ok) return `cannot verify ${constraint.provenanceKey}: ${projected.message}`;
	const expected = canonicalCollection(projected.values, constraint.comparison);
	if (!expected || !expected.every((value) => matchesIdDomain(value, constraint.idDomain))) return `cannot verify ${constraint.provenanceKey}: the projected collection is not a valid id collection`;
	const digestCollection = projectedDigestCollection(projected.values, constraint.comparison);
	if (!digestCollection) return `cannot verify ${constraint.provenanceKey}: the projected collection is outside the RFC 8785 canonical JSON domain`;
	if (constraint.relation === "equals" && constraint.allowDigest && isCanonicalDigest(actual)) return actual === require_canonicalize.canonicalDigest(digestCollection) ? null : `${constraint.provenanceKey} digest must match the projected params collection in checked encounter order`;
	if (constraint.relation === "contains" && constraint.establishesBinding === false && constraint.allowOpaqueDigestCount) {
		...
		if (opaqueCount !== void 0) return opaqueCount >= expected.length ? null : `${constraint.provenanceKey} opaque collection count (${opaqueCount}) must be at least the number of distinct observed ids (${expected.length})`;
	}
	const parsed = parseCanonicalIdArray(actual, constraint.provenanceKey, constraint.idDomain);
	if (!parsed.ok) return parsed.message;
	...
	if (constraint.relation === "equals") return require_canonicalize.canonicalize(declaredCollection) === require_canonicalize.canonicalize(expected) ? null : `${constraint.provenanceKey} must equal the projected params collection`;
	const declaredKeys = new Set(declaredCollection.map((value) => jsonScalarKey(value)));
...
function allProjectedValuesEqualError(constraint, actual, params) {
	...
	for (let index = 0; index < resolved.value.length; index++) {
		const item = resolved.value[index];
		if (item === null || typeof item !== "object" || Array.isArray(item)) return `cannot verify ${constraint.provenanceKey}: params.${constraint.paramPath}[${index}] is not an object`;
		if (!Object.hasOwn(item, constraint.field)) continue;
		const descriptor = Object.getOwnPropertyDescriptor(item, constraint.field);
		if (!descriptor || !("value" in descriptor) || !descriptor.enumerable) return `cannot verify ${constraint.provenanceKey}: params.${constraint.paramPath}[${index}].${constraint.field} is not an enumerable data property`;
...
function canonicalJsonArrayLengthError(constraint, actual, params) {
	const resolved = resolveSafeParamPath(params, constraint.paramPath);
	if (!resolved.ok || typeof resolved.value !== "number" || !Number.isSafeInteger(resolved.value) || resolved.value < 0) return `cannot verify ${constraint.provenanceKey}: params.${constraint.paramPath} is not a non-negative safe integer`;
	const declaredCount = declaredCollectionCount(actual, constraint.provenanceKey, constraint.idDomain, constraint.allowOpaqueDigestCount);
	if (!declaredCount.ok) return declaredCount.message;
	if (constraint.relation === "equals" ? declaredCount.count === resolved.value : constraint.relation === "at_least" ? declaredCount.count >= resolved.value : resolved.value === 0 || declaredCount.count >= 1) return null;
	if (constraint.relation === "nonempty_if_positive") return `${constraint.provenanceKey} collection count (${declaredCount.count}) must be at least one when params.${constraint.paramPath} is positive (${resolved.value})`;
...
function canonicalJsonArrayExactLengthError(constraint, actual) {
	if (!Number.isSafeInteger(constraint.expectedLength) || constraint.expectedLength < 0) return `cannot verify ${constraint.provenanceKey}: the contract expectedLength is not a non-negative safe integer`;
	const parsed = parseCanonicalScalarArray(actual, constraint.provenanceKey);
	if (!parsed.ok) return parsed.message;
...
function canonicalJsonArrayProjectedSumError(constraint, actual, params) {
	const projected = projectedValues(params, constraint.paramPath, constraint.field);
	if (!projected.ok) return `cannot verify ${constraint.provenanceKey}: ${projected.message}`;
	if (!projected.values.every((value) => typeof value === "number" && Number.isSafeInteger(value) && value >= 0)) return `cannot verify ${constraint.provenanceKey}: projected counts are not non-negative safe integers`;
	let minimum = 0;
	for (const value of projected.values) {
		minimum += value;
		if (!Number.isSafeInteger(minimum)) return `cannot verify ${constraint.provenanceKey}: projected count sum exceeds the safe-integer domain`;
	}
	const declaredCount = declaredCollectionCount(actual, constraint.provenanceKey, constraint.idDomain, constraint.allowOpaqueDigestCount);
	if (!declaredCount.ok) return declaredCount.message;
...
function provenanceParamConstraintError(constraint, params, declared) {
	...
	if (constraint.kind === "equals_literal") return Object.is(actual, constraint.value) ? null : `${constraint.provenanceKey} must equal ${JSON.stringify(constraint.value)}`;
	if (constraint.kind === "one_of_literals") return constraint.values.some((value) => Object.is(actual, value)) ? null : `${constraint.provenanceKey} must equal one of ${JSON.stringify(constraint.values)}`;
	if (constraint.kind === "matches_regular_time_axis") return regularTimeAxisError(constraint, actual, params);
	if (constraint.kind === "each_label_matches_variable") return seriesLabelBindingError(constraint, actual, params);
	if (constraint.kind === "matches_canonical_json_param") return canonicalJsonParamError(constraint, actual, params);
	if (constraint.kind === "matches_projected_id_collection") return projectedCollectionError(constraint, actual, params);
	if (constraint.kind === "all_projected_values_equal") return allProjectedValuesEqualError(constraint, actual, params);
	if (constraint.kind === "canonical_json_array_length_matches_param") return canonicalJsonArrayLengthError(constraint, actual, params);
	if (constraint.kind === "canonical_json_array_length_equals") return canonicalJsonArrayExactLengthError(constraint, actual);
	if (constraint.kind === "canonical_json_array_length_at_least_projected_sum") return canonicalJsonArrayProjectedSumError(constraint, actual, params);
...
const PARAM_LIMITS = Object.freeze({
	...
	maxGraphNodes: require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxPresentationNodes,
	maxGraphEdges: require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxPresentationEdges
});
const FLOAT32_MAX$1 = 34028234663852886e22;
const timeArray = zod.z.array(zod.z.number()).max(PARAM_LIMITS.maxSamples);
const gpuNumber = zod.z.number().min(-34028234663852886e22, "value exceeds the finite Float32 range used by render buffers").max(FLOAT32_MAX$1, "value exceeds the finite Float32 range used by render buffers");
const gpuArray = zod.z.array(gpuNumber).max(PARAM_LIMITS.maxSamples);
const idArray = zod.z.array(zod.z.number().int("node/sender ids must be integers").nonnegative("node/sender ids must be non-negative").max(Number.MAX_SAFE_INTEGER, "node/sender ids must be safe integers")).max(PARAM_LIMITS.maxSamples);
const displayText = (max) => zod.z.string().trim().min(1).max(max).regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN, "display text must not contain control or bidi characters").meta({ "x-cortexel-normalize": "trim" });
...
const Rfc3339TimestampSchema = zod.z.iso.datetime({ offset: true }).max(80).regex(/T\d{2}:\d{2}:\d{2}(?:\.\d+)?(?:Z|[+-]\d{2}:\d{2})$/, "timestamp must be RFC 3339 with seconds and an explicit UTC/numeric offset");
const units = displayText(80);
const normalizedRecordKey = zod.z.string().min(1).max(80).regex(/^\S(?:[\s\S]*\S)?$/, "record keys must already be trimmed and contain a non-whitespace character").regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN, "record keys must not contain control or bidi characters");
...
const VoltageTraceParamsSchema = zod.z.object({
	times_ms: timeArray.min(2),
	series: zod.z.array(gpuArray.min(1)).min(1).max(PARAM_LIMITS.maxSeries),
	series_labels: zod.z.array(displayText(120)).min(1).max(PARAM_LIMITS.maxSeries),
	...
	*  be authored as separate specs rather than sharing a misleading axis. */
	...
	value.series.forEach((series, index) => {
		equalLengthIssue(ctx, `series.${index}`, "times_ms", value.times_ms.length, series.length);
	});
	equalLengthIssue(ctx, "series_labels", "series", value.series.length, value.series_labels.length);
});
const SpikeRasterParamsSchema = zod.z.object({
	times_ms: timeArray.min(1),
	senders: idArray.min(1)
}).strict().superRefine((value, ctx) => {
	equalLengthIssue(ctx, "senders", "times_ms", value.times_ms.length, value.senders.length);
});
...
const HISTOGRAM_GEOMETRY_ABSOLUTE_TOLERANCE = 0;
const HISTOGRAM_GEOMETRY_RELATIVE_TOLERANCE = 1e-9;
/** Bounded allowance for binary64 center ± half-width edge arithmetic. */
const HISTOGRAM_GEOMETRY_ROUNDOFF_ULPS = 4;
...
const GEOMETRY_MAX_ROUNDOFF_FRACTION = 1e-7;
const HISTOGRAM_MASS_TOLERANCE = 1e-6;
const PSTH_DERIVED_COUNT_ABSOLUTE_TOLERANCE = 1e-6;
...
const POPULATION_RATE_ABSOLUTE_TOLERANCE = 1e-9;
const POPULATION_RATE_RELATIVE_TOLERANCE = 1e-9;
...
function requireUniformHistogramBins(centers, width, ctx, centerPath, nonNegativeLowerEdge = false) {
	...
	for (let index = 0; index < centers.length; index++) {
		...
		if (!Number.isFinite(left) || !Number.isFinite(right) || !(left < center) || !(center < right) || !approximatelyEqual(representedWidth, width, 0, 1e-9)) {
			ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
		...
		if (previousRight !== void 0) {
			...
			if (difference !== 0) {
				...
				const arithmeticTolerance = 4 * Number.EPSILON * Math.max(Math.abs(previousCenter), Math.abs(center), Math.abs(previousRight), Math.abs(left), Math.abs(halfWidth));
				if (arithmeticTolerance > 1e-7 * Math.abs(width) || difference > 0 + 1e-9 * Math.abs(width) + arithmeticTolerance) {
					ctx.addIssue({
						code: zod.z.ZodIssueCode.custom,
						path: [centerPath, index],
	...
	if (nonNegativeLowerEdge && centers.length > 0) {
		...
		const tolerance = 0 + HISTOGRAM_GEOMETRY_RELATIVE_TOLERANCE * Math.max(Math.abs(centers[0]), Math.abs(halfWidth));
	...
	for (let index = 1; index < centers.length; index++) {
		...
		if (!approximatelyEqual(centers[index] - centers[index - 1], width, 0, 1e-9)) {
			ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: [centerPath, index],
...
function requireNormalizedHistogramMass(normalization, values, width, rules, ctx) {
	...
	if (!approximatelyEqual(mass, rule.target, 1e-6, 1e-6)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["values"],
...
const IsiDistributionParamsSchema = zod.z.object({
	bin_centers_ms: timeArray.min(1),
	values: gpuArray.min(1),
	bin_width_ms: zod.z.number().positive().max(Number.MAX_VALUE),
	normalization: zod.z.enum([
		"count",
		"probability",
		"probability_density"
	]),
	value_units: zod.z.enum([
		"count",
		"probability",
		"1/ms"
	]),
	interval_scope: zod.z.enum(["per_sender", "single_train"])
}).strict().superRefine((value, ctx) => {
	...
	for (let index = 0; index < value.bin_centers_ms.length; index++) if (value.bin_centers_ms[index] < 0) {
		ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
	...
	if (value.value_units !== isiValueUnits[value.normalization]) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["value_units"],
	...
	requireNormalizedHistogramMass(value.normalization, value.values, value.bin_width_ms, {
		probability: {
			measure: "sum",
			target: 1
		},
		probability_density: {
			measure: "density_integral",
			target: 1
		}
	}, ctx);
});
...
function requirePsthDerivedCounts(normalization, values, trialCount, binWidthMs, ctx) {
	for (let index = 0; index < values.length; index++) {
		...
		if (!Number.isFinite(rawCount) || rawCount < 0 || !Number.isSafeInteger(rounded) || (exactCount ? !Number.isSafeInteger(rawCount) : Math.abs(rawCount - rounded) > 1e-6)) {
			ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: ["values", index],
...
const PsthParamsSchema = zod.z.object({
	bin_centers_ms: timeArray.min(1),
	values: gpuArray.min(1),
	bin_width_ms: zod.z.number().positive().max(Number.MAX_VALUE),
	normalization: zod.z.enum([
	...
	trial_count: zod.z.number().int().positive().max(Number.MAX_SAFE_INTEGER),
	alignment_event: displayText(240),
	aggregation: zod.z.literal("selected_senders_per_trial")
}).strict().superRefine((value, ctx) => {
	equalLengthIssue(ctx, "values", "bin_centers_ms", value.bin_centers_ms.length, value.values.length);
	requireMonotonic(value.bin_centers_ms, ctx, "bin_centers_ms");
	requireUniformHistogramBins(value.bin_centers_ms, value.bin_width_ms, ctx, "bin_centers_ms");
	if (value.value_units !== psthValueUnits[value.normalization]) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["value_units"],
		message: `value_units must be '${psthValueUnits[value.normalization]}' for ${value.normalization}`
	});
	for (let index = 0; index < value.values.length; index++) if (value.values[index] < 0) {
		ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["values", index],
	...
	requirePsthDerivedCounts(value.normalization, value.values, value.trial_count, value.bin_width_ms, ctx);
});
const PopulationRateSeriesSchema = zod.z.object({
	id: displayText(120),
	label: displayText(240),
	recorded_sender_count: zod.z.number().int("recorded_sender_count must be an integer").positive("recorded_sender_count must be positive").max(Number.MAX_SAFE_INTEGER, "recorded_sender_count must be a safe integer"),
	spike_counts: zod.z.array(zod.z.number().int("spike counts must be integers").nonnegative("spike counts cannot be negative").max(Number.MAX_SAFE_INTEGER, "spike counts must be safe integers")).min(1).max(PARAM_LIMITS.maxSamples),
...
function requireUniformBinWindow(centers, width, start, stop, ctx, paths = {
	...
	const edgeMatches = (edge, expected, center) => {
		...
		const arithmeticTolerance = 4 * Number.EPSILON * Math.max(Math.abs(center), Math.abs(halfWidth), Math.abs(edge), Math.abs(expected));
		if (arithmeticTolerance > 1e-7 * Math.abs(width)) return false;
		const tolerance = 0 + HISTOGRAM_GEOMETRY_RELATIVE_TOLERANCE * Math.abs(width) + arithmeticTolerance;
		return Number.isFinite(edge) && Number.isFinite(expected) && difference <= tolerance;
	};
	if (!edgeMatches(firstEdge, start, firstCenter)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
	...
	if (!edgeMatches(lastEdge, stop, lastCenter)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
...
function requirePopulationRateValues(series, binCount, binWidthMs, ctx) {
	...
	for (let seriesIndex = 0; seriesIndex < series.length; seriesIndex++) {
		...
		if (ids.has(item.id)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"series",
				seriesIndex,
		...
		ids.add(item.id);
		equalLengthIssue(ctx, `series.${seriesIndex}.spike_counts`, "bin_centers_ms", binCount, item.spike_counts.length);
		equalLengthIssue(ctx, `series.${seriesIndex}.rates_hz`, "bin_centers_ms", binCount, item.rates_hz.length);
		...
		for (let binIndex = 0; binIndex < sampleCount; binIndex++) {
			...
			if (!Number.isFinite(denominator) || !approximatelyEqual(rate, expected, 1e-9, 1e-9)) ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: [
					"series",
					seriesIndex,
...
const PopulationRateParamsSchema = zod.z.object({
	bin_centers_ms: timeArray.min(1),
	bin_width_ms: zod.z.number().positive().max(Number.MAX_VALUE),
	window_start_ms: zod.z.number(),
	window_stop_ms: zod.z.number(),
	series: zod.z.array(PopulationRateSeriesSchema).min(1).max(PARAM_LIMITS.maxSeries),
	normalization: zod.z.literal("mean_per_recorded_sender_hz"),
	aggregation: zod.z.literal("selected_senders"),
	binning: zod.z.literal("left_closed_right_open")
}).strict().superRefine((value, ctx) => {
	requireUniformHistogramBins(value.bin_centers_ms, value.bin_width_ms, ctx, "bin_centers_ms");
	requireUniformBinWindow(value.bin_centers_ms, value.bin_width_ms, value.window_start_ms, value.window_stop_ms, ctx);
	requirePopulationRateValues(value.series, value.bin_centers_ms.length, value.bin_width_ms, ctx);
});
const RateResponseParamsSchema = zod.z.object({
	stimulus_amplitudes: gpuArray.min(1),
	rates_hz: gpuArray.min(1),
	stimulus_units: units
}).strict().superRefine((value, ctx) => {
	equalLengthIssue(ctx, "rates_hz", "stimulus_amplitudes", value.stimulus_amplitudes.length, value.rates_hz.length);
	for (let index = 0; index < value.rates_hz.length; index++) if (value.rates_hz[index] < 0) {
		ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["rates_hz", index],
...
const NetworkParamsSchema = zod.z.object({
	...
	equalLengthIssue(ctx, "targets", "sources", value.sources.length, value.targets.length);
	if (value.weights) equalLengthIssue(ctx, "weights", "sources", value.sources.length, value.weights.length);
	if (value.delays) {
		equalLengthIssue(ctx, "delays", "sources", value.sources.length, value.delays.length);
		const index = value.delays.findIndex((delay) => delay <= 0);
		if (index >= 0) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["delays", index],
			message: "connection delays must be strictly positive"
		});
	}
	if (value.weights !== void 0 !== (value.weight_units !== void 0)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["weight_units"],
		message: "weight_units must be present exactly when weights are present"
	});
	if (value.delays !== void 0 !== (value.delay_units !== void 0)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["delay_units"],
		message: "delay_units must be present exactly when delays are present"
	});
});
const topologyCount = zod.z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).refine((value) => !Object.is(value, -0), "topology counts and ranks must not be negative zero");
const topologyPositiveCount = topologyCount.positive();
const MpiTargetRankLocalScopeSchema = zod.z.object({
	kind: zod.z.literal("mpi_target_rank_local"),
	rank: topologyCount,
	world_size: topologyPositiveCount
}).strict().superRefine((value, ctx) => {
	if (value.rank >= value.world_size) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["rank"],
		message: "MPI rank must be smaller than world_size"
	});
});
const SnapshotScopeSchema = zod.z.discriminatedUnion("kind", [
	zod.z.object({ kind: zod.z.literal("single_process_complete") }).strict(),
	MpiTargetRankLocalScopeSchema,
	zod.z.object({
		kind: zod.z.literal("mpi_all_ranks_merged"),
		world_size: topologyPositiveCount
	}).strict()
]);
const MpiRankLocalPositionScopeSchema = zod.z.object({
	kind: zod.z.literal("mpi_rank_local"),
	rank: topologyCount,
	world_size: topologyPositiveCount
}).strict().superRefine((value, ctx) => {
	if (value.rank >= value.world_size) ctx.addIssue({
...
const PositionScopeSchema = zod.z.discriminatedUnion("kind", [
	zod.z.object({ kind: zod.z.literal("single_process_complete") }).strict(),
...
const ConnectionGraphNodeSchema = zod.z.object({
	id: idArray.element,
	label: displayText(120)
}).strict();
const ConnectionGraphEdgeSchema = zod.z.object({
	id: displayText(240),
...
const ConnectionGraphParamsSchema = zod.z.object({
	nodes: zod.z.array(ConnectionGraphNodeSchema).min(1).max(PARAM_LIMITS.maxTopologyNodes),
	edges: zod.z.array(ConnectionGraphEdgeSchema).max(PARAM_LIMITS.maxTopologyEdges),
	weight_units: units.optional(),
	delay_units: zod.z.literal("ms").optional(),
	layout: zod.z.literal("schematic_circle"),
	parallel_edges: zod.z.literal("preserved"),
	self_connections: zod.z.literal("preserved"),
	snapshot_time_ms: zod.z.number().finite().nonnegative(),
	snapshot_scope: SnapshotScopeSchema,
	sample_policy: zod.z.enum(["complete", "deterministic_even_stride"]),
	source_connection_count: topologyCount,
	edge_identity: zod.z.enum(["nest_connection_identifier", "canonical_sorted_ordinal"])
}).strict().superRefine((value, ctx) => {
	const nodeIds = /* @__PURE__ */ new Set();
	for (let index = 0; index < value.nodes.length; index++) {
		const id = value.nodes[index].id;
		if (nodeIds.has(id)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"nodes",
				index,
	...
	for (let index = 0; index < value.edges.length; index++) {
		...
		if (edgeIds.has(edge.id)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"edges",
				index,
		...
		if (value.edge_identity === "canonical_sorted_ordinal") {
			...
			if (ordinal === void 0 || ordinal >= value.source_connection_count) ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: [
					"edges",
					index,
			...
			if (components.length !== 5 || components.some((component) => component === void 0) || components[0] !== edge.source || components[1] !== edge.target) ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: [
					"edges",
					index,
...
const matrixBaseShape = {
	...
	axis_order: zod.z.literal("target_rows_source_columns"),
	absent_cell: zod.z.literal("no_connection"),
	sample_policy: zod.z.literal("complete"),
	connection_count: topologyCount,
	snapshot_time_ms: zod.z.number().finite().nonnegative(),
	snapshot_scope: SnapshotScopeSchema
};
function refineSparseMatrix(value, ctx) {
	if (value.snapshot_scope.kind === "mpi_target_rank_local") ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
	...
	if (sourceIds.size !== value.source_ids.length) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["source_ids"],
		message: "source_ids must be unique"
	});
	if (targetIds.size !== value.target_ids.length) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["target_ids"],
		message: "target_ids must be unique"
	});
	const pairs = /* @__PURE__ */ new Set();
	let total = 0;
	for (let index = 0; index < value.cells.length; index++) {
		const cell = value.cells[index];
		if (!sourceIds.has(cell.source_id)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"cells",
				index,
		...
		if (!targetIds.has(cell.target_id)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"cells",
				index,
		...
		if (pairs.has(pair)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["cells", index],
	...
	if (total !== value.connection_count) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["connection_count"],
		message: "connection_count must equal the sum of sparse cell connection_count values"
	});
}
const AdjacencyMatrixParamsSchema = zod.z.object({
	...matrixBaseShape,
	cells: zod.z.array(AdjacencyMatrixCellSchema).max(PARAM_LIMITS.maxSamples),
	display: zod.z.literal("binary_presence"),
	aggregation: zod.z.literal("any_connection")
}).strict().superRefine(refineSparseMatrix);
const WeightMatrixParamsSchema = zod.z.object({
	...matrixBaseShape,
	cells: zod.z.array(WeightMatrixCellSchema).max(PARAM_LIMITS.maxSamples),
	weight_units: units,
...
const DelayMatrixParamsSchema = zod.z.object({
	...matrixBaseShape,
	cells: zod.z.array(DelayMatrixCellSchema).max(PARAM_LIMITS.maxSamples),
	delay_units: zod.z.literal("ms"),
	aggregation: zod.z.enum([
		"mean",
		"minimum",
		"maximum",
...
function degreeDistributionSchema(direction) {
	return zod.z.object({
		degrees: zod.z.array(topologyCount).min(1).max(PARAM_LIMITS.maxSamples),
		node_counts: zod.z.array(topologyCount).min(1).max(PARAM_LIMITS.maxSamples),
		values: gpuArray.min(1),
		node_count: topologyPositiveCount,
		connection_count: topologyCount,
		direction: zod.z.literal(direction),
		normalization: zod.z.enum(["count", "probability"]),
		value_units: zod.z.enum(["count", "probability"]),
		edge_counting: zod.z.literal("each_synapse_collection_entry"),
		zero_degree_policy: zod.z.literal("include_declared_universe"),
		sample_policy: zod.z.literal("complete"),
		snapshot_time_ms: zod.z.number().finite().nonnegative(),
		snapshot_scope: SnapshotScopeSchema
	}).strict().superRefine((value, ctx) => {
		...
		for (let index = 0; index < value.degrees.length; index++) if (value.degrees[index] !== index) {
			ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: ["degrees", index],
		...
		if (!Number.isSafeInteger(countedNodes) || countedNodes !== value.node_count) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["node_count"],
			message: "node_count must equal the sum of node_counts"
		});
		if (!Number.isSafeInteger(countedConnections) || countedConnections !== value.connection_count) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["connection_count"],
		...
		if (value.value_units !== expectedUnits) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["value_units"],
		...
		for (let index = 0; index < Math.min(value.values.length, value.node_counts.length); index++) {
			...
			if (!(value.normalization === "count" ? Number.isSafeInteger(displayed) && displayed === expected : approximatelyEqual(displayed, expected, DEGREE_VALUE_ABSOLUTE_TOLERANCE, DEGREE_VALUE_RELATIVE_TOLERANCE))) {
				ctx.addIssue({
					code: zod.z.ZodIssueCode.custom,
					path: ["values", index],
		...
		if (value.normalization === "probability" && !approximatelyEqual(displayedMass, 1, DEGREE_VALUE_ABSOLUTE_TOLERANCE, DEGREE_VALUE_RELATIVE_TOLERANCE)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["values"],
...
const InDegreeDistributionParamsSchema = degreeDistributionSchema("in");
const OutDegreeDistributionParamsSchema = degreeDistributionSchema("out");
const delayDistributionValueUnits = {
	count: "count",
	probability: "probability",
	probability_density: "1/ms"
};
const DelayDistributionParamsSchema = zod.z.object({
	bin_centers_ms: timeArray.min(1),
	delay_counts: zod.z.array(topologyCount).min(1).max(PARAM_LIMITS.maxSamples),
	values: gpuArray.min(1),
	bin_width_ms: zod.z.number().finite().positive(),
	window_start_ms: zod.z.number().finite().nonnegative(),
	window_stop_ms: zod.z.number().finite().positive(),
	normalization: zod.z.enum([
		"count",
		"probability",
		"probability_density"
	...
	delay_units: zod.z.literal("ms"),
	aggregation: zod.z.literal("each_connection"),
	binning: zod.z.literal("left_closed_right_open"),
	sample_policy: zod.z.literal("complete"),
	connection_count: topologyCount,
	snapshot_time_ms: zod.z.number().finite().nonnegative(),
	...
	if (!Number.isSafeInteger(total) || total !== value.connection_count) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["connection_count"],
		message: "connection_count must equal the sum of delay_counts"
	});
	if (value.connection_count === 0 && value.normalization !== "count") ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["normalization"],
	...
	if (value.normalization === "probability_density" && (!Number.isFinite(densityDenominator) || densityDenominator <= 0)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["bin_width_ms"],
	...
	for (let index = 0; index < Math.min(value.values.length, value.delay_counts.length); index++) {
		...
		if (!(value.normalization === "count" ? Number.isSafeInteger(displayed) && displayed === expected : Object.is(displayed, expected))) {
			ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: ["values", index],
	...
	if (value.normalization !== "count") {
		if (!approximatelyEqual(value.normalization === "probability_density" ? displayedMass * value.bin_width_ms : displayedMass, 1, 1e-6, 1e-6)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["values"],
...
const SpatialMap2DNodeSchema = zod.z.object({
	id: idArray.element,
	label: displayText(120),
	x: gpuNumber,
	y: gpuNumber
...
const SPATIAL_BOUNDS_ROUNDOFF_ULPS = 2;
function spatialBoundsTolerance(center, halfExtent, minimum, maximum) {
	...
	const arithmeticTolerance = 2 * Number.EPSILON * Math.max(Math.abs(center), Math.abs(halfExtent), Math.abs(minimum), Math.abs(maximum));
	return extentTolerance + (arithmeticTolerance <= 1e-7 * Math.abs(halfExtent) ? arithmeticTolerance : 0);
}
const SpatialMap2DParamsSchema = zod.z.object({
	nodes: zod.z.array(SpatialMap2DNodeSchema).min(1).max(PARAM_LIMITS.maxSpatialObjects),
	coordinate_units: units,
	extent: zod.z.tuple([gpuNumber.positive(), gpuNumber.positive()]),
	center: zod.z.tuple([gpuNumber, gpuNumber]),
	edge_wrap: zod.z.boolean(),
	position_scope: PositionScopeSchema,
	marker_size: zod.z.literal("fixed_screen_space")
}).strict().superRefine((value, ctx) => {
	const ids = /* @__PURE__ */ new Set();
	...
	for (let index = 0; index < value.nodes.length; index++) {
		...
		if (ids.has(node.id)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"nodes",
				index,
		...
		ids.add(node.id);
		if (node.x < minX - xTolerance || node.x > maxX + xTolerance || node.y < minY - yTolerance || node.y > maxY + yTolerance) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["nodes", index],
...
const WeightHistogramParamsSchema = zod.z.object({
	bin_centers: gpuArray.min(1),
	weight_counts: zod.z.array(topologyCount).min(1).max(PARAM_LIMITS.maxSamples),
	values: gpuArray.min(1),
	bin_width: gpuNumber.positive(),
	window_start: zod.z.number().finite(),
	window_stop: zod.z.number().finite(),
	weight_units: units,
	normalization: zod.z.enum(["count", "probability"]),
	value_units: zod.z.enum(["count", "probability"]),
	aggregation: zod.z.literal("each_connection"),
	...
	equalLengthIssue(ctx, "values", "bin_centers", value.bin_centers.length, value.values.length);
	equalLengthIssue(ctx, "weight_counts", "bin_centers", value.bin_centers.length, value.weight_counts.length);
	requireUniformHistogramBins(value.bin_centers, value.bin_width, ctx, "bin_centers");
	requireUniformBinWindow(value.bin_centers, value.bin_width, value.window_start, value.window_stop, ctx, {
		centers: "bin_centers",
		start: "window_start",
		stop: "window_stop"
	});
	if (value.value_units !== weightHistogramValueUnits[value.normalization]) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["value_units"],
	...
	for (let index = 0; index < Math.min(value.values.length, value.weight_counts.length); index++) {
		...
		if (!(value.normalization === "count" ? Number.isSafeInteger(sample) && sample === expected : Object.is(sample, expected))) {
			ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: ["values", index],
...
const Spatial3DObjectSchema = zod.z.object({
	x: gpuNumber,
	y: gpuNumber,
	z: gpuNumber
}).passthrough();
const Spatial3DParamsSchema = zod.z.object({
	objects: zod.z.array(Spatial3DObjectSchema).min(1).max(PARAM_LIMITS.maxSpatialObjects),
	coordinate_units: units
}).strict();
const PlasticityParamsSchema = zod.z.object({
	times_ms: timeArray.min(1),
	weights: gpuArray.min(1),
	weight_units: units
}).strict().superRefine((value, ctx) => {
	equalLengthIssue(ctx, "weights", "times_ms", value.times_ms.length, value.weights.length);
	requireMonotonic(value.times_ms, ctx, "times_ms");
});
const PhasePlaneDerivativeTimeUnitSchema = zod.z.enum(["ms", "s"]);
const PhasePlaneParamsSchema = zod.z.object({
	grid: zod.z.record(normalizedRecordKey, gpuArray.min(2)).refine((g) => Object.keys(g).length === 2, { message: "phase-plane grid must declare exactly two state-variable axes with at least two coordinates each" }),
	derivatives: zod.z.record(normalizedRecordKey, gpuArray.min(1)),
	axis_units: zod.z.record(normalizedRecordKey, units),
	derivative_units: zod.z.record(normalizedRecordKey, units),
	derivative_time_unit: PhasePlaneDerivativeTimeUnitSchema,
	axis_order: zod.z.tuple([normalizedRecordKey, normalizedRecordKey]).refine(([first, second]) => first !== second, { message: "axis_order must name two distinct state variables" }),
	flattening: zod.z.literal("row-major-last-axis-fastest")
}).strict().superRefine((value, ctx) => {
	const axes = Object.keys(value.grid);
	const derivativeNames = Object.keys(value.derivatives);
	for (const axis of axes) requireStrictlyIncreasing(value.grid[axis], ctx, `grid.${axis}`);
	if (value.axis_order.some((axis) => !Object.hasOwn(value.grid, axis)) || axes.some((axis) => !value.axis_order.includes(axis))) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["axis_order"],
	...
	for (const [field, values] of [["axis_units", value.axis_units], ["derivative_units", value.derivative_units]]) if (Object.keys(values).length !== axes.length || axes.some((axis) => !Object.hasOwn(values, axis))) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: [field],
		message: `${field} must declare units for the same two state variables as grid`
	});
	for (const axis of axes) if (Object.hasOwn(value.axis_units, axis) && Object.hasOwn(value.derivative_units, axis)) {
		const expected = `${value.axis_units[axis]}/${value.derivative_time_unit}`;
		if (value.derivative_units[axis] !== expected) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["derivative_units", axis],
			message: `derivative_units.${axis} must equal axis_units.${axis}/${value.derivative_time_unit}`
		});
	}
	if (value.derivative_time_unit === "s") for (const axis of derivativeNames) for (let index = 0; index < value.derivatives[axis].length; index++) {
		const derivative = value.derivatives[axis][index];
		if (derivative !== 0 && derivative / 1e3 === 0) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"derivatives",
				axis,
	...
	for (const axis of axes) equalLengthIssue(ctx, `derivatives.${axis}`, "the Cartesian phase-plane grid", expected, value.derivatives[axis].length);
});
const AstrocyteParamsSchema = zod.z.object({
	times_ms: timeArray.min(2),
	...
	* a typed analog-trace contract rather than overloading ca_trace. */
	...
	equalLengthIssue(ctx, "ca_trace", "times_ms", value.times_ms.length, value.ca_trace.length);
	requireStrictlyIncreasing(value.times_ms, ctx, "times_ms");
	for (let index = 0; index < value.ca_trace.length; index++) if (value.ca_trace[index] < 0) {
		ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["ca_trace", index],
...
const CORPUS_KNOWLEDGE_GRAPH_NODE_KINDS = [
	"paper",
	"model",
	"family"
];
const CORPUS_KNOWLEDGE_GRAPH_EDGE_KINDS = [
	"cites",
	"same_as",
	"variant_of",
	"instantiates",
...
const KnowledgeGraphAttributeScalarSchema = zod.z.union([
	zod.z.string().max(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributeStringLength).regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN, "attribute strings must not contain control or bidi characters"),
	zod.z.number(),
	zod.z.boolean(),
	zod.z.null()
]);
const KnowledgeGraphAttributeValueSchema = zod.z.union([KnowledgeGraphAttributeScalarSchema, zod.z.array(KnowledgeGraphAttributeScalarSchema).max(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributeArrayItems)]);
const KnowledgeGraphAttributeKeySchema = normalizedRecordKey.max(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributeKeyLength);
const KnowledgeGraphAttributesSchema = zod.z.record(KnowledgeGraphAttributeKeySchema, KnowledgeGraphAttributeValueSchema).superRefine((value, ctx) => {
	if (Object.keys(value).length > require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributes) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		message: `knowledge-graph attributes may contain at most ${require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributes} keys`
	});
});
const KnowledgeGraphEvidenceRefSchema = zod.z.discriminatedUnion("kind", [
	zod.z.object({
		kind: zod.z.literal("graph_snapshot_record"),
		evidence_id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxEvidenceIdLength),
		record_id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxRecordIdLength),
		locator: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxLocatorLength).optional()
	}).strict(),
	zod.z.object({
		kind: zod.z.literal("graph_node"),
		evidence_id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxEvidenceIdLength),
		node_id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxNodeIdLength),
		locator: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxLocatorLength).optional(),
		excerpt: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxExcerptLength).optional()
	}).strict(),
	zod.z.object({
		kind: zod.z.literal("citation"),
		evidence_id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxEvidenceIdLength),
		paper_id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxPaperIdLength),
		citation_id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxCitationIdLength),
		page: zod.z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).refine((value) => !Object.is(value, -0), "page must not be negative zero").optional(),
		locator: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxLocatorLength).optional(),
		excerpt: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxExcerptLength).optional(),
		doi: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxDoiLength).optional()
	}).strict(),
	zod.z.object({
		kind: zod.z.literal("external_source"),
		evidence_id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxEvidenceIdLength),
		source_id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxSourceIdLength),
		locator: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxLocatorLength).optional(),
		excerpt: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxExcerptLength).optional()
	}).strict()
]);
const KnowledgeGraphEvidenceRefsSchema = zod.z.array(KnowledgeGraphEvidenceRefSchema).min(1).max(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxEvidenceRefsPerElement).superRefine((evidence, ctx) => {
	if (!evidence.some((reference) => reference.kind !== "graph_node")) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
...
const KnowledgeGraphUncalibratedScoreSchema = zod.z.object({
	kind: zod.z.enum([
	...
	value: zod.z.number().min(0).max(1),
...
const DerivedAdvisoryEpistemicSchema = zod.z.object({
	status: zod.z.literal("derived_advisory"),
	advisory_only: zod.z.literal(true),
	is_paper_local_evidence: zod.z.literal(false),
	calibrated_posterior: zod.z.literal(false)
}).strict();
const KnowledgeGraphNodeSchema = zod.z.object({
	id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxNodeIdLength),
	kind: zod.z.enum(CORPUS_KNOWLEDGE_GRAPH_NODE_KINDS),
	label: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxNodeLabelLength),
	detail: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxDetailLength).optional(),
...
const KnowledgeGraphEdgeSchema = zod.z.object({
	id: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxEdgeIdLength),
	source: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxNodeIdLength),
	target: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxNodeIdLength),
	kind: zod.z.enum(CORPUS_KNOWLEDGE_GRAPH_EDGE_KINDS),
	label: displayText(require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxEdgeLabelLength),
	attributes: KnowledgeGraphAttributesSchema,
	epistemic: DerivedAdvisoryEpistemicSchema,
	evidence: KnowledgeGraphEvidenceRefsSchema,
	uncalibrated_score: KnowledgeGraphUncalibratedScoreSchema.optional()
}).strict();
const KnowledgeGraph3DParamsSchema = zod.z.object({
	graph_id: displayText(160),
	graph_source: displayText(200),
	graph_snapshot_id: displayText(200),
	graph_scope: zod.z.literal("corpus_entity"),
	generated_at: Rfc3339TimestampSchema,
	nodes: zod.z.array(KnowledgeGraphNodeSchema).min(1).max(PARAM_LIMITS.maxGraphNodes),
	edges: zod.z.array(KnowledgeGraphEdgeSchema).max(PARAM_LIMITS.maxGraphEdges)
}).strict().superRefine((value, ctx) => {
	const ids = /* @__PURE__ */ new Set();
	...
	value.edges.forEach((edge, index) => {
		if (edgeIds.has(edge.id)) addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"edges",
				index,
		...
		if (parallelCount > require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxParallelEdgesPerPair) addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["edges", index],
			message: `at most ${require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxParallelEdgesPerPair} parallel edges may connect one unordered node pair`
		});
		if (!ids.has(edge.source)) addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"edges",
				index,
		...
		if (!ids.has(edge.target)) addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"edges",
				index,
		...
		if (edge.source === edge.target) addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["edges", index],
		...
		const [expectedSource, expectedTarget] = {
			cites: ["paper", "paper"],
			same_as: ["model", "model"],
		...
		if (sourceKind !== void 0 && targetKind !== void 0 && (sourceKind !== expectedSource || targetKind !== expectedTarget)) addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["edges", index],
			message: `${edge.kind} requires ${expectedSource} → ${expectedTarget} endpoints`
		});
		if (edge.uncalibrated_score && !{
			cites: ["citation_resolution_confidence"],
			same_as: ["structural_similarity"],
			variant_of: ["structural_similarity"],
			instantiates: [],
			belongs_to_family: []
		}[edge.kind].includes(edge.uncalibrated_score.kind)) addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"edges",
				index,
		...
		for (let evidenceIndex = 0; evidenceIndex < edge.evidence.length; evidenceIndex++) {
			...
			if (evidenceIds.has(evidence.evidence_id)) addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: [
					"edges",
					index,
			...
			if (evidence.kind === "graph_node" && !ids.has(evidence.node_id)) addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: [
					"edges",
					index,
	...
	value.nodes.forEach((node, nodeIndex) => {
		if (node.uncalibrated_score && node.uncalibrated_score.kind !== "extraction_confidence") addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"nodes",
				nodeIndex,
...
const Spatial2DParamsSchema = zod.z.object({
	positions: zod.z.array(zod.z.tuple([gpuNumber, gpuNumber])).min(1).max(PARAM_LIMITS.maxSpatialObjects),
	coordinate_units: units
}).strict();
const CorrelogramPairSchema = zod.z.object({
	reference_label: displayText(240),
	target_label: displayText(240)
}).strict();
const CorrelogramStatisticSchema = zod.z.discriminatedUnion("kind", [
	zod.z.object({
		kind: zod.z.literal("raw_pair_count"),
		units: zod.z.literal("count")
	}).strict(),
	zod.z.object({
		kind: zod.z.literal("weighted_pair_sum"),
		units
	}).strict(),
	zod.z.object({
		kind: zod.z.literal("pair_rate_hz"),
		units: zod.z.literal("Hz"),
		exposure_s: zod.z.number().positive().max(Number.MAX_VALUE)
	}).strict(),
	zod.z.object({
		kind: zod.z.literal("pearson_coefficient"),
		units: zod.z.literal("1"),
		sample_count: zod.z.number().int().positive().max(Number.MAX_SAFE_INTEGER)
	}).strict()
]);
function requireSymmetricLagAxis(lags, width, tauMax, ctx) {
	...
	if (!approximatelyEqual(lags[0], -tauMax, 0, 1e-9)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["lags_ms", 0],
		message: "the first lag center must equal -tau_max_ms"
	});
	const lastIndex = lags.length - 1;
	if (!approximatelyEqual(lags[lastIndex], tauMax, 0, 1e-9)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["lags_ms", lastIndex],
		message: "the final lag center must equal tau_max_ms"
	});
	const middle = Math.floor(lags.length / 2);
	if (!approximatelyEqual(lags[middle], 0, 0, 1e-9)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["lags_ms", middle],
		message: "the middle lag center must be zero"
	});
	for (let index = 0; index < middle; index++) if (!approximatelyEqual(lags[index], -lags[lastIndex - index], 0, 1e-9)) {
		ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: ["lags_ms", index],
...
const CorrelogramParamsSchema = zod.z.object({
	lags_ms: timeArray.min(1),
	values: gpuArray.min(1),
	bin_width_ms: zod.z.number().positive().max(Number.MAX_VALUE),
	tau_max_ms: zod.z.number().positive().max(Number.MAX_VALUE),
	counting_start_ms: zod.z.number(),
	counting_stop_ms: zod.z.number(),
	pair: CorrelogramPairSchema,
	lag_convention: zod.z.literal("positive_target_after_reference"),
	binning: zod.z.literal("left_closed_right_open"),
	zero_lag_policy: zod.z.enum(["included", "excluded_self_pairs"]),
	statistic: CorrelogramStatisticSchema
}).strict().superRefine((value, ctx) => {
	equalLengthIssue(ctx, "values", "lags_ms", value.lags_ms.length, value.values.length);
	requireSymmetricLagAxis(value.lags_ms, value.bin_width_ms, value.tau_max_ms, ctx);
	if (!(value.counting_stop_ms > value.counting_start_ms)) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
	...
	for (let index = 0; index < value.values.length; index++) {
		const sample = value.values[index];
		if (value.statistic.kind === "pearson_coefficient" ? sample < -1 || sample > 1 : value.statistic.kind === "raw_pair_count" ? sample < 0 || !Number.isSafeInteger(sample) : value.statistic.kind === "pair_rate_hz" ? sample < 0 : false) {
			ctx.addIssue({
				code: zod.z.ZodIssueCode.custom,
				path: ["values", index],
...
const StimulusResponseParamsSchema = zod.z.object({
	times_ms: timeArray.min(1),
	stimulus: gpuArray.min(1),
	response: gpuArray.min(1)
}).strict().superRefine((value, ctx) => {
	equalLengthIssue(ctx, "stimulus", "times_ms", value.times_ms.length, value.stimulus.length);
	equalLengthIssue(ctx, "response", "times_ms", value.times_ms.length, value.response.length);
	requireMonotonic(value.times_ms, ctx, "times_ms");
});
const CompartmentalParamsSchema = zod.z.object({
	times_ms: timeArray.min(2),
	compartments: zod.z.array(zod.z.object({
		id: displayText(120),
		parent_id: displayText(120).nullable(),
		label: displayText(240).optional(),
		values: gpuArray.min(1)
	}).strict()).min(1).max(PARAM_LIMITS.maxSeries)
}).strict().superRefine((value, ctx) => {
	requireStrictlyIncreasing(value.times_ms, ctx, "times_ms");
	const ids = /* @__PURE__ */ new Set();
	...
	value.compartments.forEach((compartment, index) => {
		if (ids.has(compartment.id)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"compartments",
				index,
		...
		equalLengthIssue(ctx, `compartments.${index}.values`, "times_ms", value.times_ms.length, compartment.values.length);
	});
	if (roots === 0) ctx.addIssue({
		code: zod.z.ZodIssueCode.custom,
		path: ["compartments"],
		message: "at least one root compartment must have parent_id:null"
	});
	value.compartments.forEach((compartment, index) => {
		if (compartment.parent_id !== null && !ids.has(compartment.parent_id)) ctx.addIssue({
			code: zod.z.ZodIssueCode.custom,
			path: [
				"compartments",
				index,
...
const AnimationReplayParamsSchema = zod.z.object({ frames: zod.z.array(zod.z.object({
	time_ms: zod.z.number().nonnegative(),
	state: zod.z.record(normalizedRecordKey, zod.z.unknown()).refine((state) => Object.keys(state).length > 0, { message: "frame state must contain at least one field" }),
	annotation: displayText(500).optional()
}).strict()).min(1).max(1e4) }).strict().superRefine((value, ctx) => {
	requireMonotonic(value.frames.map((frame) => frame.time_ms), ctx, "frames.time_ms");
});

//#endregion
//#region core/skills/examples.ts
const synthetic = (declared_inputs) => ({
...
const HOST_RENDERER_EXAMPLE_PAYLOADS = {
	"nest.spatial_2d": {
...
for (const payload of Object.values(HOST_RENDERER_EXAMPLE_PAYLOADS)) if (payload) deepFreezeJson(payload);
...
function verificationKindForConstraints(constraints) {
	if (constraints.every((constraint) => constraint.kind === "equals_literal" || constraint.kind === "one_of_literals")) return "literal_bound";
	if (constraints.every((constraint) => constraint.kind === "equals_param" || constraint.kind === "equals_param_path")) return "param_bound";
	return "derived_bound";
}
function provenanceVerificationForContract(contract) {
	const verification = Object.create(null);
	const classifiedKeys = [...contract.requiredProvenanceKeys, ...contract.optionalProvenanceKeys ?? []];
	for (const key of classifiedKeys) {
		const constraints = (contract.provenanceParamConstraints ?? []).filter((constraint) => constraint.provenanceKey === key && constraintEstablishesBinding(constraint));
		const external = contract.externalProvenanceClaims?.[key];
		if (constraints.length > 0 === (external !== void 0)) throw new Error(`skill '${contract.id}' must classify required provenance '${key}' exactly once as mechanically bound or external`);
...
const CORTEXEL_SKILL_VERSION = "1.8.0";
const STRICT_INVOCATION_POLICY = Object.freeze({
	version: "3",
...
*  `[*]`, `*`, or `?` are interpreted. */
const PARAM_CONSTRAINT_LANGUAGE = Object.freeze({
	version: "11",
...
const NEST_SKILL_REGISTRY = {
	"nest.voltage_trace": {
		...
		externalProvenanceClaims: externalClaims({ device_id: { reason: "The multimeter/voltmeter source-device identity is not represented in trace params." } }),
	...
	"nest.spike_raster": {
		...
		externalProvenanceClaims: externalClaims({
			recorder_id: { reason: "The spike-recorder source identity is not represented in event params." },
			sender_ids: { reason: "Observed events cannot establish the complete recorded-sender universe because silent senders disappear." },
			population_labels: { reason: "Event params carry sender ids but no source population-identity mapping." }
		}),
		provenanceParamConstraints: [{
			kind: "equals_literal",
	...
	"nest.isi_distribution": {
		...
		externalProvenanceClaims: externalClaims({
			recorder_id: { reason: "The source recorder identity is not retained by the derived histogram params." },
			sender_ids: { reason: "ISI aggregation does not retain the selected sender universe." },
			population_labels: { reason: "ISI aggregation does not retain population identity or membership." }
		}),
		provenanceParamConstraints: [
			{
				kind: "equals_literal",
	...
	"nest.psth": {
		...
		externalProvenanceClaims: externalClaims({
			recorder_id: { reason: "The source recorder identity is not retained by PSTH params." },
			sender_ids: { reason: "PSTH aggregation retains recoverable counts but not selected sender identities." },
			population_labels: { reason: "PSTH params do not retain a population-identity mapping." }
		}),
		provenanceParamConstraints: [
			{
				kind: "equals_literal",
	...
	"nest.population_rate": {
		...
		externalProvenanceClaims: externalClaims({
			recorder_id: { reason: "The source recorder identity is not represented in rate params." },
			sender_ids: { reason: "Per-series sender counts do not establish the identities of the selected senders." },
			population_labels: { reason: "Series display ids/labels are not a structured source population-identity mapping." }
		}),
		provenanceParamConstraints: [
			{
	...
	"nest.rate_response": {
		...
		externalProvenanceClaims: externalClaims({
			bin_ms: { reason: "The response params contain no observation window, raw counts, or bin axis from which to verify this duration." },
			rate_normalization: { reason: "The response params contain rates only, without the denominator or derivation needed to verify normalization." }
		}),
		provenanceParamConstraints: [{
			kind: "equals_param",
	...
	"nest.connectivity_matrix": {
		...
		externalProvenanceClaims: externalClaims({
			source_ids: { reason: "The deprecated edge list retains observed endpoints, not the complete selected source universe." },
			target_ids: { reason: "The deprecated edge list retains observed endpoints, not the complete selected target universe." },
			synapse_model: { reason: "The deprecated edge-list params do not retain a snapshot-level synapse model." },
			connection_sample_policy: { reason: "The deprecated edge-list params do not retain a sampling/completeness policy." }
		}),
		provenanceParamConstraints: [
			{
				kind: "matches_projected_id_collection",
	...
	"nest.connection_graph": {
		...
		externalProvenanceClaims: externalClaims({
			source_ids: { reason: "Graph params preserve a role-erased node union; observed edges cannot establish isolated selected sources." },
			target_ids: { reason: "Graph params preserve a role-erased node union; observed edges cannot establish isolated selected targets." },
			synapse_model: { reason: "Edge-level model values can prevent contradictions when present but do not establish a snapshot-level model for empty or model-omitting graphs." }
		}),
		provenanceParamConstraints: [
			{
				kind: "matches_projected_id_collection",
	...
	"nest.adjacency_matrix": {
		...
		routerEligibility: {
			bareFamilyCandidate: true,
			dataShapeKind: "adjacency_matrix"
		},
		transform: {
		...
		externalProvenanceClaims: externalClaims({ synapse_model: { reason: "Adjacency-matrix params do not retain the snapshot synapse model." } }),
		provenanceParamConstraints: [
			{
			...
			},
			{
				...
				provenanceKey: "connection_sample_policy",
				paramKey: "sample_policy",
				description: "Only complete connection snapshots may form a literal matrix."
			},
			{
				kind: "equals_param",
				provenanceKey: "snapshot_time_ms",
				paramKey: "snapshot_time_ms",
				description: "Declared snapshot time must match params."
			},
			{
				kind: "equals_param_path",
				provenanceKey: "snapshot_scope",
				paramPath: "snapshot_scope.kind",
				description: "Declared snapshot scope must match params."
			},
			{
				kind: "equals_literal",
				provenanceKey: "parallel_edge_policy",
				value: "preserved_as_connection_count",
				description: "Parallel connections are preserved as each sparse cell count."
			},
			{
				kind: "equals_param",
				provenanceKey: "matrix_axis_order",
				paramKey: "axis_order",
				description: "Declared matrix axes must match params."
			},
			{
				kind: "equals_param",
				provenanceKey: "matrix_aggregation",
				paramKey: "aggregation",
				description: "Declared matrix aggregation must match params."
			}
		],
		rendererRoutes: [
			"media.trace_figure",
	...
	"nest.weight_matrix": {
		...
		externalProvenanceClaims: externalClaims({ synapse_model: { reason: "Weight-matrix params do not retain the snapshot synapse model." } }),
		provenanceParamConstraints: [
			{
				kind: "matches_canonical_json_param",
				provenanceKey: "source_ids",
			...
			},
			{
				...
				provenanceKey: "weight_units",
				paramKey: "weight_units",
				description: "Weight units must match params."
			},
			{
				kind: "equals_param",
				provenanceKey: "connection_sample_policy",
			...
			},
			{
				...
				provenanceKey: "snapshot_time_ms",
				paramKey: "snapshot_time_ms",
				description: "Snapshot time must match params."
			},
			{
				kind: "equals_param_path",
				provenanceKey: "snapshot_scope",
				paramPath: "snapshot_scope.kind",
				description: "Snapshot scope must match params."
			},
			{
				kind: "equals_literal",
				provenanceKey: "parallel_edge_policy",
				value: "preserved_as_connection_count",
				description: "Parallel connections remain auditable via connection_count."
			},
			{
				kind: "equals_param",
				provenanceKey: "matrix_axis_order",
				paramKey: "axis_order",
				description: "Matrix axis order must match params."
			},
			{
				kind: "equals_param",
				provenanceKey: "matrix_aggregation",
				paramKey: "aggregation",
				description: "Weight aggregation must match params."
			}
		],
		rendererRoutes: [
			"media.trace_figure",
	...
	"nest.delay_matrix": {
		...
		externalProvenanceClaims: externalClaims({ synapse_model: { reason: "Delay-matrix params do not retain the snapshot synapse model." } }),
		provenanceParamConstraints: [
			{
				kind: "matches_canonical_json_param",
				provenanceKey: "source_ids",
			...
			},
			{
				...
				provenanceKey: "delay_units",
				paramKey: "delay_units",
				description: "Delay units must match params."
			},
			{
				kind: "equals_param",
				provenanceKey: "connection_sample_policy",
			...
			},
			{
				...
				provenanceKey: "matrix_aggregation",
				paramKey: "aggregation",
				description: "Delay aggregation must match params."
			}
		],
		rendererRoutes: [
			"media.trace_figure",
	...
	"nest.in_degree_distribution": {
		...
		routerEligibility: {
			bareFamilyCandidate: true,
			dataShapeKind: "in_degree_distribution"
		},
		transform: {
		...
		externalProvenanceClaims: externalClaims({
			source_ids: { reason: "The aggregate degree distribution does not retain source identities." },
			target_ids: { reason: "The aggregate degree distribution retains a target count but not target identities." },
			synapse_model: { reason: "The aggregate degree params do not retain the snapshot synapse model." }
		}),
		provenanceParamConstraints: [
			{
			...
			},
			{
				...
				provenanceKey: "connection_sample_policy",
				paramKey: "sample_policy",
				description: "Degree input must be complete for its declared scope."
			},
			{
				kind: "equals_param",
				provenanceKey: "snapshot_time_ms",
			...
			},
			{
				...
				description: "Every multapse contributes one degree edge."
			},
			{
				kind: "equals_param",
				provenanceKey: "degree_direction",
				paramKey: "direction",
				description: "Degree direction must match params."
			},
			{
				kind: "equals_param",
				provenanceKey: "degree_counting",
				paramKey: "edge_counting",
				description: "Degree counting must match params."
			},
			{
				kind: "equals_param",
				provenanceKey: "zero_degree_policy",
				paramKey: "zero_degree_policy",
				description: "Zero-degree policy must match params."
			},
			{
				kind: "equals_param",
				provenanceKey: "histogram_normalization",
				paramKey: "normalization",
				description: "Degree normalization must match params."
			}
		],
		rendererRoutes: [
			"media.trace_figure",
	...
	"nest.out_degree_distribution": {
		...
		routerEligibility: {
			bareFamilyCandidate: true,
			dataShapeKind: "out_degree_distribution"
		},
		transform: {
		...
		externalProvenanceClaims: externalClaims({
			source_ids: { reason: "The aggregate degree distribution retains a source count but not source identities." },
			target_ids: { reason: "The aggregate degree distribution does not retain target identities." },
			synapse_model: { reason: "The aggregate degree params do not retain the snapshot synapse model." }
		}),
		provenanceParamConstraints: [
			{
	...
	"nest.delay_distribution": {
		...
		routerEligibility: {
			bareFamilyCandidate: true,
			dataShapeKind: "delay_distribution"
		},
		transform: {
		...
		externalProvenanceClaims: externalClaims({
			source_ids: { reason: "The aggregate delay histogram does not retain source identities." },
			target_ids: { reason: "The aggregate delay histogram does not retain target identities." },
			synapse_model: { reason: "The aggregate delay params do not retain the snapshot synapse model." }
		}),
		provenanceParamConstraints: [
			{
				kind: "canonical_json_array_length_matches_param",
			...
			},
			{
				...
				provenanceKey: "connection_sample_policy",
				paramKey: "sample_policy",
				description: "Delay histogram input must be complete for its scope."
			},
			{
				kind: "equals_param",
				provenanceKey: "snapshot_time_ms",
			...
			},
			{
				...
				provenanceKey: "parallel_edge_policy",
				value: "count_each_connection",
				description: "Every selected connection contributes one delay."
			},
			{
				kind: "equals_param",
				provenanceKey: "bin_ms",
				paramKey: "bin_width_ms",
				description: "Delay bin width must match params."
			},
			{
				kind: "equals_param",
				provenanceKey: "histogram_normalization",
				paramKey: "normalization",
				description: "Delay normalization must match params."
			},
			{
				kind: "equals_param",
				provenanceKey: "binning_policy",
				paramKey: "binning",
				description: "Delay binning policy must match params."
			}
		],
		rendererRoutes: [
			"media.trace_figure",
	...
	"nest.weight_histogram": {
		...
		externalProvenanceClaims: externalClaims({
			source_ids: { reason: "The aggregate weight histogram does not retain source identities." },
			target_ids: { reason: "The aggregate weight histogram does not retain target identities." },
			synapse_model: { reason: "The aggregate weight params do not retain the snapshot synapse model." },
			parallel_edge_policy: { reason: "The serialized histogram retains aggregate counts but no raw-entry derivation receipt from which to authenticate the claimed one-entry/one-observation mapping." }
		}),
		provenanceParamConstraints: [
			{
				kind: "canonical_json_array_length_matches_param",
	...
	"nest.spatial_2d": {
		...
		externalProvenanceClaims: externalClaims({
			extent: { reason: "Anonymous point bounds are not the declared layer extent and params contain no center/extent object." },
			mask: { reason: "The network-generation mask is source configuration, not measured position data." },
			kernel: { reason: "The network-generation kernel is source configuration, not measured position data." }
		}),
		provenanceParamConstraints: [{
	...
	"nest.spatial_3d": {
		...
		externalProvenanceClaims: externalClaims({
			extent: { reason: "Position bounds are not a layer extent and params contain no center/extent object." },
			projection_sample_policy: { reason: "The positioned-object params do not retain a structured projection sampling policy." }
		}),
		provenanceParamConstraints: [{
			kind: "canonical_json_array_length_equals",
			provenanceKey: "extent",
	...
	"nest.phase_plane": {
		...
		externalProvenanceClaims: externalClaims({
			derivation_method: { reason: "The vector-field params contain derivative values but no structured derivation method/version." },
			model_context: { reason: "The vector-field params contain no structured model identity/version." },
			fixed_parameters: { reason: "The vector-field params do not retain the fixed-parameter map and units." }
		}),
		provenanceParamConstraints: [{
			kind: "matches_canonical_json_param",
	...
	"nest.correlogram": {
		...
		externalProvenanceClaims: externalClaims({
			detector_id: { reason: "The source correlation-detector identity is not represented in correlogram params." },
			reference_population: { reason: "The reference label is caller supplied and does not authenticate which external population was wired to detector receptor port 0." },
			target_population: { reason: "The target label is caller supplied and does not authenticate which external population was wired to detector receptor port 1." }
		}),
		provenanceParamConstraints: [
			{
				kind: "equals_param",
	...
	"nest.stimulus_response": {
		...
		externalProvenanceClaims: externalClaims({
			stim_units: { reason: "Stimulus-response params contain an untyped stimulus array without a unit field." },
			units: { reason: "Stimulus-response params contain an untyped response array without a unit field." }
		}),
		provenanceParamConstraints: [{
			kind: "equals_literal",
			provenanceKey: "time_units",
	...
	"nest.astrocyte_dynamics": {
		...
		externalProvenanceClaims: externalClaims({ recorded_variable: { reason: "The legacy ca_trace field does not distinguish the NEST Ca and Ca_astro source-variable names." } }),
		provenanceParamConstraints: [
			{
				kind: "equals_param",
				provenanceKey: "units",
	...
	"nest.compartmental_dynamics": {
		...
		externalProvenanceClaims: externalClaims({
			morphology_disclaimer: { reason: "Morphology geometry is absent; this caller text cannot substitute for a contract-owned geometry disclosure." },
			recorded_variable: { reason: "Compartment traces do not carry a structured shared/per-series recorded variable." },
			units: { reason: "Compartment traces do not carry a structured shared/per-series unit." }
		}),
		provenanceParamConstraints: [{
			kind: "equals_literal",
			provenanceKey: "time_units",
	...
	"nest.animation_replay": {
		...
		externalProvenanceClaims: externalClaims({ frame_rate: { reason: "Playback frame rate is not derivable from simulation timestamps without an explicit playback time scale." } }),
...
const PARAM_VALIDATION_CONSTRAINTS = {
	"nest.voltage_trace": [
		{
			kind: "equal_length",
	...
	"nest.adjacency_matrix": [{
		...
		kind: "scope_compatibility",
		paths: ["snapshot_scope"],
		allowedFieldValues: ["single_process_complete", "mpi_all_ranks_merged"],
	...
	"corpus.knowledge_graph": [
		...
		},
		{
			...
			paths: ["edges"],
			max: require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxParallelEdgesPerPair,
		...
		},
		{
			...
			max: require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributes,
...
for (const contract of Object.values(NEST_SKILL_REGISTRY)) contract.paramConstraints = PARAM_VALIDATION_CONSTRAINTS[contract.id];
/** Neutral alias for the skill registry (the axis is not NEST-only — see
*  corpus.knowledge_graph). Prefer this in new code. */
const SKILL_REGISTRY = NEST_SKILL_REGISTRY;
...
for (const contract of Object.values(NEST_SKILL_REGISTRY)) {
	...
	const allowedConstraintKeys = /* @__PURE__ */ new Set([...contract.requiredProvenanceKeys, ...contract.optionalProvenanceKeys ?? []]);
	for (const constraint of contract.provenanceParamConstraints ?? []) if (!allowedConstraintKeys.has(constraint.provenanceKey)) throw new Error(`skill '${contract.id}' constraint targets unclassified provenance '${constraint.provenanceKey}'`);
	for (const key of Object.keys(contract.externalProvenanceClaims ?? {})) if (!contract.requiredProvenanceKeys.includes(key)) throw new Error(`skill '${contract.id}' external provenance '${key}' is not required`);
	...
	if (contract.provenanceParamConstraints) Object.freeze(contract.provenanceParamConstraints);
...
function getSkill(id) {
	return isSkillId(id) ? NEST_SKILL_REGISTRY[id] : void 0;
}
...
function annotatePortableStringRules(value) {
	...
	const schema = value;
	if (schema.type === "string" && typeof schema.maxLength === "number") schema["x-cortexel-max-utf16-code-units"] = schema.maxLength;
...
function toPortableJsonSchema(schemaSource) {
	const schema = zod.z.toJSONSchema(schemaSource, { io: "input" });
...
function describeSkill(id) {
	...
	return {
		...
		provenanceParamConstraints: (c.provenanceParamConstraints ?? []).map((constraint) => ({
			...constraint,
		...
		paramConstraints: (c.paramConstraints ?? []).map((constraint) => ({
			...
			...constraint.allowedEndpointKinds ? { allowedEndpointKinds: Object.fromEntries(Object.entries(constraint.allowedEndpointKinds).map(([kind, pair]) => [kind, [...pair]])) } : {},
			...constraint.normalizationRules ? { normalizationRules: Object.fromEntries(Object.entries(constraint.normalizationRules).map(([mode, rule]) => [mode, { ...rule }])) } : {},
			...constraint.allowedValues ? { allowedValues: { ...constraint.allowedValues } } : {},
			...constraint.numericDomains ? { numericDomains: Object.fromEntries(Object.entries(constraint.numericDomains).map(([mode, domain]) => [mode, { ...domain }])) } : {},
			...constraint.allowedScoreKinds ? { allowedScoreKinds: Object.fromEntries(Object.entries(constraint.allowedScoreKinds).map(([kind, scoreKinds]) => [kind, [...scoreKinds]])) } : {},
...
//#endregion
//#region core/skills/paramPreflight.ts
...
const INVOCATION_FIELDS = /* @__PURE__ */ new Set([
	"scene",
	"skill",
	"specVersion",
	"params",
...
const PROVENANCE_FIELDS = /* @__PURE__ */ new Set([
	"source",
...
const gpu = (value) => finite(value) && Math.abs(value) <= FLOAT32_MAX;
const id = (value) => typeof value === "number" && Number.isSafeInteger(value) && value >= 0 && !Object.is(value, -0);
...
function largeArray(value, path, check, expected, options = {}) {
	...
	for (let index = 0; index < value.length; index++) if (!check(value[index])) return {
		path: `${path}.${index}`,
...
function preflightLargeSkillParams(skillId, params) {
	...
	switch (skillId) {
		case "nest.spike_raster": return numericFields(params, [time(), idField("senders")]);
		case "nest.isi_distribution": {
			const issue = numericFields(params, [time("bin_centers_ms"), gpuField("values")]);
			if (issue) return issue;
			for (const [field, message] of [["bin_centers_ms", "inter-spike interval bin centers cannot be negative"], ["values", "histogram values cannot be negative"]]) {
				const values = params[field];
				if (Array.isArray(values)) {
					const index = values.findIndex((value) => typeof value === "number" && value < 0);
					if (index >= 0) return {
						path: `${field}.${index}`,
						message
					};
		...
		case "nest.population_rate": {
			...
			const outer = largeArray(params.series, "series", (series) => {
				const item = record(series);
				return !!item && Object.keys(item).every((key) => [
					"id",
					"label",
					"recorded_sender_count",
					"spike_counts",
					"rates_hz"
				].includes(key)) && boundedText(item.id, 120) && boundedText(item.label, 240) && typeof item.recorded_sender_count === "number" && Number.isSafeInteger(item.recorded_sender_count) && item.recorded_sender_count > 0 && Array.isArray(item.spike_counts) && item.spike_counts.length >= 1 && Array.isArray(item.rates_hz) && item.rates_hz.length >= 1;
			}, "a closed population-rate series with id, label, sender count, spike counts, and rates", { max: PARAM_LIMITS.maxSeries });
			if (outer) return outer;
			for (let index = 0; index < params.series.length; index++) {
				const item = record(params.series[index]);
				if (!item) continue;
				const counts = largeArray(item.spike_counts, `series.${index}.spike_counts`, (value) => id(value), "a non-negative safe-integer spike count");
				if (counts) return counts;
				const rates = largeArray(item.rates_hz, `series.${index}.rates_hz`, (value) => gpu(value) && value >= 0, "a non-negative finite Float32-range rate");
				if (rates) return rates;
			}
			return null;
		}
		case "nest.rate_response": {
			const issue = numericFields(params, [gpuField("stimulus_amplitudes"), gpuField("rates_hz")]);
			if (issue) return issue;
		...
		case "nest.connectivity_matrix": return numericFields(params, [
			idField("sources"),
			idField("targets"),
			gpuField("weights"),
			gpuField("delays")
		...
		case "nest.connection_graph": {
			const nodes = largeArray(params.nodes, "nodes", (value) => {
				const node = record(value);
				return !!node && id(node.id) && boundedText(node.label, 120);
			}, "a closed graph node with a safe id and bounded label", { max: PARAM_LIMITS.maxTopologyNodes });
			if (nodes) return nodes;
			return largeArray(params.edges, "edges", (value) => {
		...
		case "nest.delay_matrix": {
			...
			if (axes) return axes;
			return largeArray(params.cells, "cells", (value) => {
		...
		case "nest.out_degree_distribution": return numericFields(params, [
			idField("degrees"),
			idField("node_counts"),
			gpuField("values")
		]);
		case "nest.delay_distribution": return numericFields(params, [
			time("bin_centers_ms"),
		...
		case "nest.plasticity_dynamics": return numericFields(params, [time(), gpuField("weights")]);
		...
		case "nest.correlogram": return numericFields(params, [time("lags_ms"), gpuField("values")]);
		case "nest.stimulus_response": return numericFields(params, [
			time(),
			gpuField("stimulus"),
			gpuField("response")
		]);
		case "nest.voltage_trace": {
			...
			if (Array.isArray(params.series)) {
				const outer = largeArray(params.series, "series", (series) => Array.isArray(series) && series.length >= 1 && series.length <= MAX_SAMPLES, "a non-empty numeric series", { max: 256 });
				if (outer) return outer;
				for (let index = 0; index < params.series.length; index++) {
					const nested = largeArray(params.series[index], `series.${index}`, gpu, "a finite Float32-range number");
					if (nested) return nested;
				}
			}
			const labels = largeArray(params.series_labels, "series_labels", (label) => boundedText(label, 120), "a bounded non-blank label", { max: 256 });
		...
		case "nest.phase_plane":
			for (const field of ["grid", "derivatives"]) {
				...
				for (const [name, values] of Object.entries(collection)) {
					const issue = largeArray(values, `${field}.${name}`, gpu, "a finite Float32-range number");
					if (issue) return issue;
				}
			}
			return null;
		case "nest.spatial_2d": return largeArray(params.positions, "positions", (position) => Array.isArray(position) && position.length === 2 && position.every(gpu), "an exact [x,y] Float32-range tuple", { max: PARAM_LIMITS.maxSpatialObjects });
		case "nest.spatial_map_2d": return largeArray(params.nodes, "nodes", (value) => {
			const node = record(value);
			return !!node && id(node.id) && boundedText(node.label, 120) && gpu(node.x) && gpu(node.y);
		}, "an identified 2D node with finite coordinates", { max: PARAM_LIMITS.maxSpatialObjects });
		case "nest.spatial_3d": return largeArray(params.objects, "objects", (object) => {
			const item = record(object);
			return !!item && gpu(item.x) && gpu(item.y) && gpu(item.z);
		}, "an object with finite Float32-range x/y/z", { max: PARAM_LIMITS.maxSpatialObjects });
		case "nest.compartmental_dynamics": {
			const issue = numericFields(params, [time()]);
			if (issue) return issue;
			if (Array.isArray(params.compartments)) {
				const outer = largeArray(params.compartments, "compartments", (compartment) => {
					...
					return Object.keys(item).every((key) => [
						...
						"label",
						"values"
					].includes(key)) && boundedText(item.id, 120) && (item.parent_id === null || boundedText(item.parent_id, 120)) && (item.label === void 0 || boundedText(item.label, 240)) && Array.isArray(item.values) && item.values.length >= 1;
				}, "a closed compartment with id, parent_id, and non-empty values", { max: 256 });
				if (outer) return outer;
				for (let index = 0; index < params.compartments.length; index++) {
					const item = record(params.compartments[index]);
					if (!item) continue;
					const nested = largeArray(item.values, `compartments.${index}.values`, gpu, "a finite Float32-range number");
					if (nested) return nested;
				}
			}
			return null;
		}
		case "nest.animation_replay": return largeArray(params.frames, "frames", (frame) => {
			...
			return !!item && Object.keys(item).every((key) => [
				...
				"state",
				"annotation"
			].includes(key)) && finite(item.time_ms) && item.time_ms >= 0 && !!state && Object.keys(state).length > 0 && Object.keys(state).every((key) => key.length >= 1 && key.length <= 80 && key.trim() === key) && (item.annotation === void 0 || boundedText(item.annotation, 500));
		}, "a frame with non-negative time_ms and an object state", { max: 1e4 });
		case "corpus.knowledge_graph": {
			...
			const attributesAreBounded = (value) => {
				...
				if (!attributes || Object.keys(attributes).length > require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributes) return false;
				return Object.values(attributes).every((attribute) => !Array.isArray(attribute) || attribute.length <= require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributeArrayItems);
			};
			const evidenceIsBounded = (value) => Array.isArray(value) && value.length >= 1 && value.length <= require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxEvidenceRefsPerElement;
			...
			return largeArray(params.nodes, "nodes", (node) => {
				...
				return !!item && Object.keys(item).every((key) => [
					...
					"evidence",
					"uncalibrated_score"
				].includes(key)) && boundedText(item.id, 120) && boundedText(item.label, 240) && (item.detail === void 0 || boundedText(item.detail, require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxDetailLength)) && attributesAreBounded(item.attributes) && epistemicIsDerived(item.epistemic) && evidenceIsBounded(item.evidence) && nodeKinds.has(item.kind);
			}, "a bounded, evidence-carrying paper/model/family node", { max: PARAM_LIMITS.maxGraphNodes }) ?? largeArray(params.edges, "edges", (edge) => {
				...
				min: 0,
				max: PARAM_LIMITS.maxGraphEdges
			});
		}
		default: return null;
	}
...
function preflightRawSkillParams(skillId, params) {
	...
	if (allowed) {
		...
		if (Reflect.ownKeys(params).some((field) => typeof field !== "string" || !allowedSet.has(field))) return {
			path: "(root)",
	...
	const tooLongValue = (value, path, max) => {
		...
		return length !== void 0 && length > max ? {
			path,
			message: `${path} may contain at most ${max} items`
		} : null;
	};
	...
	const tooManyKeys = (key, max) => {
		...
		return Reflect.ownKeys(value).length > max ? {
			path: key,
			message: `${key} may contain at most ${max} properties`
		} : null;
	};
	...
	const nestedArrays = (outerKey, outerMax, innerKey) => {
		...
		for (let index = 0; index < length; index++) {
			...
			if (!itemDescriptor || !("value" in itemDescriptor) || !itemDescriptor.enumerable) continue;
			...
			const issue = tooLongValue(nested, innerKey ? `${outerKey}.${index}.${innerKey}` : `${outerKey}.${index}`, MAX_SAMPLES);
			if (issue) return issue;
		}
		return null;
	};
	const recordValueArrays = (key) => {
		...
		if (collection === null || typeof collection !== "object" || Array.isArray(collection)) return null;
		const keys = Reflect.ownKeys(collection);
		if (keys.length > 2) return {
			path: key,
			message: `${key} may contain at most 2 properties`
		};
		...
		for (let index = 0; index < length; index++) {
			...
			if (attributes !== null && typeof attributes === "object" && !Array.isArray(attributes)) {
				...
				for (const attributeKey in attributes) {
					...
					attributeCount += 1;
					if (attributeCount > require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributes) return {
						path: `${key}.${index}.attributes`,
						message: `attributes may contain at most ${require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributes} properties`
					};
					...
					const valueIssue = tooLongValue(attributeDescriptor.value, `${key}.${index}.attributes.${attributeKey}`, require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxAttributeArrayItems);
			...
			const evidenceIssue = tooLongValue(evidence, `${key}.${index}.evidence`, require_knowledgeGraphLimits.KNOWLEDGE_GRAPH_LIMITS.maxEvidenceRefsPerElement);
			if (evidenceIssue) return evidenceIssue;
		}
		return null;
	};
	switch (skillId) {
		case "nest.voltage_trace": return directArrays(["times_ms"]) ?? nestedArrays("series", PARAM_LIMITS.maxSeries) ?? tooLong("series_labels", PARAM_LIMITS.maxSeries);
		case "nest.spike_raster": return directArrays(["times_ms", "senders"]);
		case "nest.isi_distribution":
		case "nest.psth": return directArrays(["bin_centers_ms", "values"]);
		case "nest.population_rate": return directArrays(["bin_centers_ms"]) ?? nestedArrays("series", PARAM_LIMITS.maxSeries, "spike_counts") ?? nestedArrays("series", PARAM_LIMITS.maxSeries, "rates_hz");
		case "nest.rate_response": return directArrays(["stimulus_amplitudes", "rates_hz"]);
		case "nest.connectivity_matrix": return directArrays([
			"sources",
			"targets",
			"weights",
			"delays"
		]);
		case "nest.connection_graph": return tooLong("nodes", PARAM_LIMITS.maxTopologyNodes) ?? tooLong("edges", PARAM_LIMITS.maxTopologyEdges);
		case "nest.adjacency_matrix":
		case "nest.weight_matrix":
		case "nest.delay_matrix": return directArrays(["source_ids", "target_ids"]) ?? tooLong("cells", PARAM_LIMITS.maxSamples);
		case "nest.in_degree_distribution":
		case "nest.out_degree_distribution": return directArrays([
			"degrees",
			"node_counts",
			"values"
		]);
		case "nest.delay_distribution": return directArrays([
			"bin_centers_ms",
			"delay_counts",
			"values"
		]);
		case "nest.weight_histogram": return directArrays([
			"bin_centers",
			"weight_counts",
			"values"
		]);
		case "nest.spatial_2d": return tooLong("positions", PARAM_LIMITS.maxSpatialObjects);
		case "nest.spatial_map_2d": return tooLong("nodes", PARAM_LIMITS.maxSpatialObjects);
		case "nest.spatial_3d": return tooLong("objects", PARAM_LIMITS.maxSpatialObjects);
		case "nest.plasticity_dynamics": return directArrays(["times_ms", "weights"]);
		case "nest.compartmental_dynamics": return directArrays(["times_ms"]) ?? nestedArrays("compartments", PARAM_LIMITS.maxSeries, "values");
		case "nest.correlogram": return directArrays(["lags_ms", "values"]);
		case "nest.stimulus_response": return directArrays([
			"times_ms",
			"stimulus",
			"response"
		]);
		case "nest.astrocyte_dynamics": return directArrays(["times_ms", "ca_trace"]);
		case "nest.animation_replay": return tooLong("frames", 1e4);
		case "corpus.knowledge_graph": return graphElementBudgets("nodes", PARAM_LIMITS.maxGraphNodes) ?? graphElementBudgets("edges", PARAM_LIMITS.maxGraphEdges);
		case "nest.phase_plane": return tooManyKeys("grid", 2) ?? tooManyKeys("derivatives", 2) ?? tooManyKeys("axis_units", 2) ?? tooManyKeys("derivative_units", 2) ?? recordValueArrays("grid") ?? recordValueArrays("derivatives") ?? tooLong("axis_order", 2);
		default: return null;
	}
}
function preflightRawEnvelopeParams(skillId, payload) {
	...
	if (prototype !== Object.prototype && prototype !== null) return null;
	if (Reflect.ownKeys(payload).some((field) => typeof field !== "string" || !INVOCATION_FIELDS.has(field))) return {
		scope: "envelope",
		path: "(root)",
	...
	if (provenance && "value" in provenance && provenance.enumerable && provenance.value !== null && typeof provenance.value === "object" && !Array.isArray(provenance.value)) {
		...
		if (provenancePrototype === Object.prototype || provenancePrototype === null) {
			if (Reflect.ownKeys(provenance.value).some((field) => typeof field !== "string" || !PROVENANCE_FIELDS.has(field))) return {
				scope: "envelope",
				path: "provenance",
...
//#endregion
//#region core/skills/validateSkillInvocation.ts
...
function nearestSkill(id) {
	if (id.length === 0 || id.length > 80) return void 0;
	let best;
	let bestD = Infinity;
	...
	return best !== void 0 && bestD <= Math.max(3, Math.ceil(id.length * .4)) ? best : void 0;
}
	...
	try {
		...
		if (!contract) {
			...
			return {
				...
				errors: [{
					code: "unknown_skill",
					path: "skillId",
					message: `unknown skill '${require_knowledgeGraphLimits.safePrimitiveDiagnostic(skillId)}'`,
		...
		if (!json.success) return {
			...
			errors: json.error.issues.slice(0, MAX_INVOCATION_ERRORS).map((issue, index) => {
				const bounded = require_knowledgeGraphLimits.boundValidationIssue(issue);
				return {
					code: "invalid_params",
		...
		if (!contract.paramsSchema) return {
			ok: false,
			errors: [{
				code: "invalid_params",
				path: "params",
		...
		return {
			...
			errors: [{
				code: "invalid_params",
				path: "params",
				message: `validation could not safely inspect params: ${require_knowledgeGraphLimits.safeErrorMessage(error)}`
			}]
		};
	}
}
function validateSkillInvocationUnsafe(skillId, payload) {
	...
	const rawProvenance = require_knowledgeGraphLimits.readOwnEnumerableDataProperty(payload, "provenance");
	const rawCalibrated = rawProvenance.kind === "value" ? require_knowledgeGraphLimits.readOwnEnumerableDataProperty(rawProvenance.value, "calibrated_posterior") : { kind: "absent" };
	if (rawCalibrated.kind === "value" && rawCalibrated.value === true) return {
		ok: false,
		errors: [{
	...
	const rawVersionProperty = require_knowledgeGraphLimits.readOwnEnumerableDataProperty(payload, "specVersion");
	const rawVersion = rawVersionProperty.kind === "value" ? rawVersionProperty.value : void 0;
	if (rawVersionProperty.kind === "value" && rawVersion !== "1.5.0") return {
		ok: false,
		errors: [{
			code: "unsupported_spec_version",
			path: "specVersion",
			message: `unsupported spec version '${require_knowledgeGraphLimits.safePrimitiveDiagnostic(rawVersion)}'`,
	...
	if (!envelope.ok) return {
		...
		errors: envelope.errors.slice(0, MAX_INVOCATION_ERRORS).map((formatted, index) => {
			...
			return {
				...
				path: separator >= 0 ? formatted.slice(0, separator) : "(spec)",
				message: separator >= 0 ? formatted.slice(separator + 2) : formatted,
	...
	if (spec.skill && spec.skill !== skillId) errors.push({
		code: "skill_mismatch",
		path: "skill",
	...
	if (spec.scene !== contract.scene) errors.push({
		code: "scene_mismatch",
		path: "scene",
	...
	if (contract.paramsSchema) {
		...
		if (preflight) errors.push({
			...
			if (issues.length > detailedCount && errors.length < MAX_INVOCATION_ERRORS) errors.push({
				code: "invalid_params",
				path: "params.(root)",
	...
	for (const flag of [
		...
		if (required !== void 0 && prov[flag] !== required) errors.push({
	...
	const declared = normalizeDeclaredProvenanceInputs(prov.declared_inputs ?? {});
	...
	const allowedDeclaredKeys = /* @__PURE__ */ new Set([...contract.requiredProvenanceKeys, ...contract.optionalProvenanceKeys ?? []]);
	...
	if (!errors.some((error) => error.code === "invalid_params")) for (const constraint of contract.provenanceParamConstraints ?? []) {
		if (errors.length >= MAX_INVOCATION_ERRORS) break;
		if (invalidDeclaredKeys.has(constraint.provenanceKey)) continue;
		const message = provenanceParamConstraintError(constraint, spec.params, declared);
	...
	if (spec.palette && !isRegisteredPalette(spec.palette)) errors.push({
	...
	if (contract.weak) weakDisclosure = contract.weakDisclosure ?? `Derived view — ${skillId} reuses the '${contract.scene}' scene; not a 1:1 rendering.`;
	...
	try {
		...
		return {
			...
			errors: [{
				code: "invalid_envelope",
				path: "(spec)",
				message: `validation could not safely inspect the payload: ${require_knowledgeGraphLimits.safeErrorMessage(error)}`,
				validScenes: SCENE_NAMES
			}]
		};
	}
...
//#endregion
//#region core/skills/hostInvocation.ts
const HostRendererInvocationSchema = zod.z.object({
	skill: zod.z.string().trim().min(1).max(80).regex(require_knowledgeGraphLimits.SAFE_DISPLAY_STRING_PATTERN, "skill must not contain display control characters"),
	specVersion: zod.z.literal(CORTEXEL_SPEC_VERSION).optional(),
	...
	*  every route the skill contract permits. */
	rendererRoute: zod.z.enum(VALID_RENDERER_ROUTES).optional()
}).strict();
...
function validateHostRendererInvocationUnsafe(skillId, payload) {
	...
	if (spec.skill !== contract.id) errors.push({
		code: "skill_mismatch",
		path: "skill",
	...
	if (!params.ok) for (const error of params.errors.slice(0, MAX_HOST_ERRORS - errors.length)) errors.push({
		...error,
	...
	for (const flag of [
		...
		const required = contract.requiredProvenanceFlags?.[flag];
		if (required !== void 0 && spec.provenance[flag] !== required) errors.push({
			code: "invalid_provenance",
			path: `provenance.${flag}`,
	...
	const declared = normalizeDeclaredProvenanceInputs(spec.provenance.declared_inputs ?? {});
	if (spec.provenance.declared_inputs) spec = {
		...spec,
		provenance: {
			...spec.provenance,
			declared_inputs: declared
	...
	if (params.ok) for (const constraint of contract.provenanceParamConstraints ?? []) {
		if (errors.length >= MAX_HOST_ERRORS) break;
		if (invalidDeclaredKeys.has(constraint.provenanceKey)) continue;
		const message = provenanceParamConstraintError(constraint, spec.params, declared);
		if (message) errors.push({
	...
	if (spec.rendererRoute !== void 0 && !contract.rendererRoutes.includes(spec.rendererRoute)) errors.push({
	...
	const externalDisclosure = externalProvenanceDisclosure(contract);
	let weakDisclosure = null;
	if (contract.weak) weakDisclosure = contract.weakDisclosure ?? `Derived host view — '${contract.id}' is not a native Cortexel scene.`;
	...
	try {
		...
		return {
			...
			errors: [{
				code: "invalid_envelope",
				path: "(spec)",
				message: `host invocation could not be safely inspected: ${require_knowledgeGraphLimits.safeErrorMessage(error)}`,
				validSkills: NEST_SKILL_IDS
			}]
		};
	}
...
function validateHostRendererSpec(payload) {
	let skillProperty;
	try {
		skillProperty = require_knowledgeGraphLimits.readOwnEnumerableDataProperty(payload, "skill");
	} catch (error) {
		return {
			ok: false,
			errors: [{
				code: "invalid_envelope",
				path: "(spec)",
				message: `validation could not safely inspect spec.skill: ${require_knowledgeGraphLimits.safeErrorMessage(error)}`
			}]
		};
	}
	...
	if (typeof normalizedSkill !== "string" || normalizedSkill.length === 0) return {
		ok: false,
		errors: [{
			code: "unknown_skill",
			path: "skill",
...
//#endregion
//#region core/skills/authoring.ts
const ALLOWED_PROVENANCE_OVERRIDES = /* @__PURE__ */ new Set([
	"advisory_only",
	"is_paper_local_evidence",
	"synthetic",
	"caption"
]);
const BUILD_VIZSPEC_FIELDS = /* @__PURE__ */ new Set([
	"skill",
	"params",
	"source",
	"declaredInputs",
...
const BUILD_HOST_FIELDS = /* @__PURE__ */ new Set([
	"skill",
	"params",
	"source",
	"declaredInputs",
...
function safeAuthoringField(value) {
	return require_knowledgeGraphLimits.safeDiagnosticText(value, 120);
}
function safeAuthoringPath(path, field) {
	return require_knowledgeGraphLimits.safeDiagnosticText(field === void 0 ? path : `${path}.${safeAuthoringField(field)}`, require_knowledgeGraphLimits.PUBLIC_DIAGNOSTIC_LIMITS.maxPathLength);
}
function safeAuthoringMessage(value) {
	return require_knowledgeGraphLimits.safeDiagnosticText(value, require_knowledgeGraphLimits.PUBLIC_DIAGNOSTIC_LIMITS.maxMessageLength);
}
...
function normalizeProvenanceOverrides(override) {
	...
	const snapshot = snapshotAuthoringObject(override, ALLOWED_PROVENANCE_OVERRIDES, "provenance");
	return snapshot.ok ? {
		ok: true,
		overrides: snapshot.value
	} : snapshot;
}
/**
...
function buildVizSpecUnsafe(input) {
	...
	if (contract && contract.scene === null) return {
		ok: false,
		errors: [{
			code: "no_cortexel_scene",
			path: "skillId",
	...
	try {
		...
		return {
			...
			errors: [{
				code: "invalid_envelope",
				path: "(input)",
				message: `host authoring could not safely inspect the input: ${require_knowledgeGraphLimits.safeErrorMessage(error)}`
			}]
		};
	}
}
	...
	try {
		...
		return {
			...
			errors: [{
				code: "invalid_envelope",
				path: "(input)",
				message: `authoring could not safely inspect the input: ${require_knowledgeGraphLimits.safeErrorMessage(error)}`
			}]
		};
	}
}
...
function formatInvocationErrors(errors) {
	try {
		...
		let example;
		if (exampleCandidate !== void 0) try {
			if (JSON.stringify(exampleCandidate).length <= 25e3) example = exampleCandidate;
		} catch {}
		const boundedErrors = errors.slice(0, require_knowledgeGraphLimits.PUBLIC_DIAGNOSTIC_LIMITS.maxIssues).map((error) => ({
			code: error.code,
			path: require_knowledgeGraphLimits.safeDiagnosticText(error.path, require_knowledgeGraphLimits.PUBLIC_DIAGNOSTIC_LIMITS.maxPathLength),
			message: require_knowledgeGraphLimits.safeDiagnosticText(error.message, require_knowledgeGraphLimits.PUBLIC_DIAGNOSTIC_LIMITS.maxMessageLength),
			...error.hint ? { hint: require_knowledgeGraphLimits.safeDiagnosticText(error.hint, require_knowledgeGraphLimits.PUBLIC_DIAGNOSTIC_LIMITS.maxMessageLength) } : {},
			...error.didYouMean ? { didYouMean: require_knowledgeGraphLimits.safeDiagnosticText(error.didYouMean, 80) } : {},
			...error.validSkills ? { validSkills: error.validSkills.slice(0, 100).map((value) => require_knowledgeGraphLimits.safeDiagnosticText(value, 80)) } : {},
			...error.validScenes ? { validScenes: error.validScenes.slice(0, 100) } : {},
			...error.validPalettes ? { validPalettes: error.validPalettes.slice(0, 100).map((value) => require_knowledgeGraphLimits.safeDiagnosticText(value, 80)) } : {}
		}));
...
//#endregion
Object.defineProperty(exports, 'AXIS_COLORS', {
  enumerable: true,
  get: function () {
    return AXIS_COLORS;
  }
});
Object.defineProperty(exports, 'AdjacencyMatrixParamsSchema', {
  enumerable: true,
  get: function () {
    return AdjacencyMatrixParamsSchema;
  }
});
Object.defineProperty(exports, 'AnimationReplayParamsSchema', {
  enumerable: true,
  get: function () {
    return AnimationReplayParamsSchema;
  }
});
Object.defineProperty(exports, 'AstrocyteParamsSchema', {
  enumerable: true,
  get: function () {
    return AstrocyteParamsSchema;
  }
});
Object.defineProperty(exports, 'BATLOW_GLSL', {
  enumerable: true,
  get: function () {
    return BATLOW_GLSL;
  }
...
Object.defineProperty(exports, 'CONSERVATIVE_PROVENANCE', {
  enumerable: true,
  get: function () {
    return CONSERVATIVE_PROVENANCE;
  }
});
Object.defineProperty(exports, 'CORPUS_KNOWLEDGE_GRAPH_EDGE_KINDS', {
  enumerable: true,
  get: function () {
    return CORPUS_KNOWLEDGE_GRAPH_EDGE_KINDS;
  }
});
Object.defineProperty(exports, 'CORPUS_KNOWLEDGE_GRAPH_NODE_KINDS', {
  enumerable: true,
  get: function () {
    return CORPUS_KNOWLEDGE_GRAPH_NODE_KINDS;
  }
});
Object.defineProperty(exports, 'CORTEXEL_JSON_LIMITS', {
  enumerable: true,
  get: function () {
    return CORTEXEL_JSON_LIMITS;
  }
});
Object.defineProperty(exports, 'CORTEXEL_JSON_POLICY', {
  enumerable: true,
  get: function () {
    return CORTEXEL_JSON_POLICY;
  }
});
Object.defineProperty(exports, 'CORTEXEL_PALETTE', {
  enumerable: true,
  get: function () {
    return CORTEXEL_PALETTE;
  }
});
Object.defineProperty(exports, 'CORTEXEL_SKILL_VERSION', {
  enumerable: true,
  get: function () {
    return CORTEXEL_SKILL_VERSION;
  }
});
Object.defineProperty(exports, 'CORTEXEL_SPEC_VERSION', {
  enumerable: true,
  get: function () {
    return CORTEXEL_SPEC_VERSION;
  }
});
Object.defineProperty(exports, 'CORTICAL_LAYER_COLORS', {
  enumerable: true,
  get: function () {
    return CORTICAL_LAYER_COLORS;
  }
});
Object.defineProperty(exports, 'CompartmentalParamsSchema', {
  enumerable: true,
  get: function () {
    return CompartmentalParamsSchema;
  }
});
Object.defineProperty(exports, 'ConnectionGraphParamsSchema', {
  enumerable: true,
  get: function () {
    return ConnectionGraphParamsSchema;
  }
});
Object.defineProperty(exports, 'CorrelogramParamsSchema', {
  enumerable: true,
  get: function () {
    return CorrelogramParamsSchema;
  }
});
Object.defineProperty(exports, 'DECLARED_INPUTS_PORTABLE_SCHEMA', {
  enumerable: true,
  get: function () {
    return DECLARED_INPUTS_PORTABLE_SCHEMA;
  }
});
Object.defineProperty(exports, 'DelayDistributionParamsSchema', {
  enumerable: true,
  get: function () {
    return DelayDistributionParamsSchema;
  }
});
Object.defineProperty(exports, 'DelayMatrixParamsSchema', {
  enumerable: true,
  get: function () {
    return DelayMatrixParamsSchema;
  }
});
Object.defineProperty(exports, 'ENVELOPE_NORMALIZATION_POLICY', {
  enumerable: true,
  get: function () {
    return ENVELOPE_NORMALIZATION_POLICY;
  }
});
Object.defineProperty(exports, 'GEOMETRY_MAX_ROUNDOFF_FRACTION', {
  enumerable: true,
  get: function () {
    return GEOMETRY_MAX_ROUNDOFF_FRACTION;
  }
});
Object.defineProperty(exports, 'HISTOGRAM_GEOMETRY_ABSOLUTE_TOLERANCE', {
  enumerable: true,
  get: function () {
    return HISTOGRAM_GEOMETRY_ABSOLUTE_TOLERANCE;
  }
});
Object.defineProperty(exports, 'HISTOGRAM_GEOMETRY_RELATIVE_TOLERANCE', {
  enumerable: true,
  get: function () {
    return HISTOGRAM_GEOMETRY_RELATIVE_TOLERANCE;
  }
});
Object.defineProperty(exports, 'HISTOGRAM_GEOMETRY_ROUNDOFF_ULPS', {
  enumerable: true,
  get: function () {
    return HISTOGRAM_GEOMETRY_ROUNDOFF_ULPS;
  }
});
Object.defineProperty(exports, 'HISTOGRAM_MASS_TOLERANCE', {
  enumerable: true,
  get: function () {
    return HISTOGRAM_MASS_TOLERANCE;
  }
});
Object.defineProperty(exports, 'HONESTY_POLICY', {
  enumerable: true,
  get: function () {
    return HONESTY_POLICY;
  }
});
Object.defineProperty(exports, 'HOST_RENDERER_EXAMPLE_PAYLOADS', {
  enumerable: true,
  get: function () {
    return HOST_RENDERER_EXAMPLE_PAYLOADS;
  }
});
Object.defineProperty(exports, 'HostRendererInvocationSchema', {
  enumerable: true,
  get: function () {
    return HostRendererInvocationSchema;
  }
});
Object.defineProperty(exports, 'InDegreeDistributionParamsSchema', {
  enumerable: true,
  get: function () {
    return InDegreeDistributionParamsSchema;
  }
});
Object.defineProperty(exports, 'IsiDistributionParamsSchema', {
  enumerable: true,
  get: function () {
    return IsiDistributionParamsSchema;
  }
});
Object.defineProperty(exports, 'JSON_BUDGET_SEMANTICS', {
  enumerable: true,
  get: function () {
    return JSON_BUDGET_SEMANTICS;
  }
});
Object.defineProperty(exports, 'JSON_PARAMS_PORTABLE_SCHEMA', {
  enumerable: true,
  get: function () {
    return JSON_PARAMS_PORTABLE_SCHEMA;
  }
});
Object.defineProperty(exports, 'JsonParamsSchema', {
  enumerable: true,
  get: function () {
    return JsonParamsSchema;
  }
});
Object.defineProperty(exports, 'KnowledgeGraph3DParamsSchema', {
  enumerable: true,
  get: function () {
    return KnowledgeGraph3DParamsSchema;
  }
});
Object.defineProperty(exports, 'NEST_DEVICE_FAMILIES', {
  enumerable: true,
  get: function () {
    return NEST_DEVICE_FAMILIES;
  }
});
Object.defineProperty(exports, 'NEST_SKILL_IDS', {
  enumerable: true,
  get: function () {
    return NEST_SKILL_IDS;
  }
});
Object.defineProperty(exports, 'NEST_SKILL_REGISTRY', {
  enumerable: true,
  get: function () {
    return NEST_SKILL_REGISTRY;
  }
});
Object.defineProperty(exports, 'NUMERIC_MODEL_POLICY', {
  enumerable: true,
  get: function () {
    return NUMERIC_MODEL_POLICY;
  }
});
Object.defineProperty(exports, 'NetworkParamsSchema', {
  enumerable: true,
  get: function () {
    return NetworkParamsSchema;
  }
});
Object.defineProperty(exports, 'OKABE_ITO', {
  enumerable: true,
  get: function () {
    return OKABE_ITO;
  }
});
Object.defineProperty(exports, 'OutDegreeDistributionParamsSchema', {
  enumerable: true,
  get: function () {
    return OutDegreeDistributionParamsSchema;
  }
});
Object.defineProperty(exports, 'PALETTE_REGISTRY_POLICY', {
  enumerable: true,
  get: function () {
    return PALETTE_REGISTRY_POLICY;
  }
});
Object.defineProperty(exports, 'PARAM_CONSTRAINT_LANGUAGE', {
  enumerable: true,
  get: function () {
    return PARAM_CONSTRAINT_LANGUAGE;
  }
});
Object.defineProperty(exports, 'PARAM_LIMITS', {
  enumerable: true,
  get: function () {
    return PARAM_LIMITS;
  }
});
Object.defineProperty(exports, 'PARAM_VALIDATION_CONSTRAINTS', {
  enumerable: true,
  get: function () {
    return PARAM_VALIDATION_CONSTRAINTS;
  }
});
Object.defineProperty(exports, 'POPULATION_RATE_ABSOLUTE_TOLERANCE', {
  enumerable: true,
  get: function () {
    return POPULATION_RATE_ABSOLUTE_TOLERANCE;
  }
});
Object.defineProperty(exports, 'POPULATION_RATE_RELATIVE_TOLERANCE', {
  enumerable: true,
  get: function () {
    return POPULATION_RATE_RELATIVE_TOLERANCE;
  }
});
Object.defineProperty(exports, 'PROVENANCE_KEYS', {
  enumerable: true,
  get: function () {
    return PROVENANCE_KEYS;
  }
});
Object.defineProperty(exports, 'PROVENANCE_KEY_LABELS', {
  enumerable: true,
  get: function () {
    return PROVENANCE_KEY_LABELS;
  }
});
Object.defineProperty(exports, 'PROVENANCE_PARAM_CONSTRAINT_LANGUAGE', {
  enumerable: true,
  get: function () {
    return PROVENANCE_PARAM_CONSTRAINT_LANGUAGE;
  }
});
Object.defineProperty(exports, 'PROVENANCE_VALUE_CONSTRAINTS', {
  enumerable: true,
  get: function () {
    return PROVENANCE_VALUE_CONSTRAINTS;
  }
});
Object.defineProperty(exports, 'PSTH_DERIVED_COUNT_ABSOLUTE_TOLERANCE', {
  enumerable: true,
  get: function () {
    return PSTH_DERIVED_COUNT_ABSOLUTE_TOLERANCE;
  }
});
Object.defineProperty(exports, 'PhasePlaneParamsSchema', {
  enumerable: true,
  get: function () {
    return PhasePlaneParamsSchema;
  }
});
Object.defineProperty(exports, 'PlasticityParamsSchema', {
  enumerable: true,
  get: function () {
    return PlasticityParamsSchema;
  }
});
Object.defineProperty(exports, 'PopulationRateParamsSchema', {
  enumerable: true,
  get: function () {
    return PopulationRateParamsSchema;
  }
});
Object.defineProperty(exports, 'PositionScopeSchema', {
  enumerable: true,
  get: function () {
    return PositionScopeSchema;
  }
});
Object.defineProperty(exports, 'ProvenanceKeyEnum', {
  enumerable: true,
  get: function () {
    return ProvenanceKeyEnum;
  }
});
Object.defineProperty(exports, 'ProvenanceSchema', {
  enumerable: true,
  get: function () {
    return ProvenanceSchema;
  }
});
Object.defineProperty(exports, 'PsthParamsSchema', {
  enumerable: true,
  get: function () {
    return PsthParamsSchema;
  }
});
Object.defineProperty(exports, 'RateResponseParamsSchema', {
  enumerable: true,
  get: function () {
    return RateResponseParamsSchema;
  }
});
Object.defineProperty(exports, 'Rfc3339TimestampSchema', {
  enumerable: true,
  get: function () {
    return Rfc3339TimestampSchema;
  }
});
Object.defineProperty(exports, 'SCENE_FRAMING', {
  enumerable: true,
  get: function () {
    return SCENE_FRAMING;
  }
});
Object.defineProperty(exports, 'SCENE_NAMES', {
  enumerable: true,
  get: function () {
    return SCENE_NAMES;
  }
});
Object.defineProperty(exports, 'SEMANTIC_PALETTE_KEYS', {
  enumerable: true,
  get: function () {
    return SEMANTIC_PALETTE_KEYS;
  }
});
Object.defineProperty(exports, 'SKILL_EXAMPLE_PAYLOADS', {
  enumerable: true,
  get: function () {
    return SKILL_EXAMPLE_PAYLOADS;
  }
});
Object.defineProperty(exports, 'SKILL_IDS', {
  enumerable: true,
  get: function () {
    return SKILL_IDS;
  }
...
Object.defineProperty(exports, 'SPATIAL_BOUNDS_ROUNDOFF_ULPS', {
  enumerable: true,
  get: function () {
    return SPATIAL_BOUNDS_ROUNDOFF_ULPS;
  }
});
Object.defineProperty(exports, 'STRICT_INVOCATION_POLICY', {
  enumerable: true,
  get: function () {
    return STRICT_INVOCATION_POLICY;
  }
});
Object.defineProperty(exports, 'STRICT_PROVENANCE_POLICY', {
  enumerable: true,
  get: function () {
    return STRICT_PROVENANCE_POLICY;
  }
});
Object.defineProperty(exports, 'STRING_NORMALIZATION_POLICY', {
  enumerable: true,
  get: function () {
    return STRING_NORMALIZATION_POLICY;
  }
});
Object.defineProperty(exports, 'SYNAPSE_COLORS', {
  enumerable: true,
  get: function () {
    return SYNAPSE_COLORS;
  }
});
Object.defineProperty(exports, 'SnapshotScopeSchema', {
  enumerable: true,
  get: function () {
    return SnapshotScopeSchema;
  }
});
Object.defineProperty(exports, 'Spatial2DParamsSchema', {
  enumerable: true,
  get: function () {
    return Spatial2DParamsSchema;
  }
});
Object.defineProperty(exports, 'Spatial3DParamsSchema', {
  enumerable: true,
  get: function () {
    return Spatial3DParamsSchema;
  }
});
Object.defineProperty(exports, 'SpatialMap2DParamsSchema', {
  enumerable: true,
  get: function () {
    return SpatialMap2DParamsSchema;
  }
});
Object.defineProperty(exports, 'SpikeRasterParamsSchema', {
  enumerable: true,
  get: function () {
    return SpikeRasterParamsSchema;
  }
});
Object.defineProperty(exports, 'StimulusResponseParamsSchema', {
  enumerable: true,
  get: function () {
    return StimulusResponseParamsSchema;
  }
});
Object.defineProperty(exports, 'TURBO_GLSL', {
  enumerable: true,
  get: function () {
    return TURBO_GLSL;
  }
});
Object.defineProperty(exports, 'VALID_RENDERER_ROUTES', {
  enumerable: true,
  get: function () {
    return VALID_RENDERER_ROUTES;
  }
});
Object.defineProperty(exports, 'VIK_GLSL', {
  enumerable: true,
  get: function () {
    return VIK_GLSL;
  }
});
Object.defineProperty(exports, 'VIRIDIS_GLSL', {
  enumerable: true,
  get: function () {
    return VIRIDIS_GLSL;
  }
});
Object.defineProperty(exports, 'VIZ_ROUTER_ID', {
  enumerable: true,
  get: function () {
    return VIZ_ROUTER_ID;
  }
});
Object.defineProperty(exports, 'VizSpecSchema', {
  enumerable: true,
  get: function () {
    return VizSpecSchema;
  }
});
Object.defineProperty(exports, 'VoltageTraceParamsSchema', {
  enumerable: true,
  get: function () {
    return VoltageTraceParamsSchema;
  }
});
Object.defineProperty(exports, 'WeightHistogramParamsSchema', {
  enumerable: true,
  get: function () {
    return WeightHistogramParamsSchema;
  }
});
Object.defineProperty(exports, 'WeightMatrixParamsSchema', {
  enumerable: true,
  get: function () {
    return WeightMatrixParamsSchema;
  }
});
Object.defineProperty(exports, 'buildHostRendererInvocation', {
  enumerable: true,
  get: function () {
    return buildHostRendererInvocation;
  }
});
Object.defineProperty(exports, 'buildVizSpec', {
  enumerable: true,
  get: function () {
    return buildVizSpec;
  }
...
Object.defineProperty(exports, 'colormapGradient', {
  enumerable: true,
  get: function () {
    return colormapGradient;
  }
});
Object.defineProperty(exports, 'colormapHex', {
  enumerable: true,
  get: function () {
    return colormapHex;
  }
});
Object.defineProperty(exports, 'colormapRgba', {
  enumerable: true,
  get: function () {
    return colormapRgba;
  }
});
Object.defineProperty(exports, 'colormapSvgStops', {
  enumerable: true,
  get: function () {
    return colormapSvgStops;
  }
});
Object.defineProperty(exports, 'composeHonestyCaption', {
  enumerable: true,
  get: function () {
    return composeHonestyCaption;
  }
});
Object.defineProperty(exports, 'conservativeProvenance', {
  enumerable: true,
  get: function () {
    return conservativeProvenance;
  }
});
Object.defineProperty(exports, 'declaredProvenanceValueError', {
  enumerable: true,
  get: function () {
    return declaredProvenanceValueError;
  }
});
Object.defineProperty(exports, 'defaultHonestyCaption', {
  enumerable: true,
  get: function () {
    return defaultHonestyCaption;
  }
});
Object.defineProperty(exports, 'describeSkill', {
  enumerable: true,
  get: function () {
    return describeSkill;
  }
});
Object.defineProperty(exports, 'describeSkills', {
  enumerable: true,
  get: function () {
    return describeSkills;
  }
});
Object.defineProperty(exports, 'externalProvenanceDisclosure', {
  enumerable: true,
  get: function () {
    return externalProvenanceDisclosure;
  }
});
Object.defineProperty(exports, 'formatInvocationErrors', {
  enumerable: true,
  get: function () {
    return formatInvocationErrors;
  }
});
Object.defineProperty(exports, 'getExamplePayload', {
  enumerable: true,
  get: function () {
    return getExamplePayload;
  }
});
Object.defineProperty(exports, 'getHostRendererExamplePayload', {
  enumerable: true,
  get: function () {
    return getHostRendererExamplePayload;
  }
});
Object.defineProperty(exports, 'getInvocationExamplePayload', {
  enumerable: true,
  get: function () {
    return getInvocationExamplePayload;
  }
});
...
Object.defineProperty(exports, 'getPaletteEntry', {
  enumerable: true,
  get: function () {
...
Object.defineProperty(exports, 'isNestSkillId', {
  enumerable: true,
  get: function () {
    return isNestSkillId;
  }
});
Object.defineProperty(exports, 'isProvenanceKey', {
  enumerable: true,
  get: function () {
    return isProvenanceKey;
  }
});
Object.defineProperty(exports, 'isRegisteredPalette', {
  enumerable: true,
  get: function () {
    return isRegisteredPalette;
  }
});
Object.defineProperty(exports, 'isSkillId', {
  enumerable: true,
  get: function () {
    return isSkillId;
  }
});
Object.defineProperty(exports, 'listPalettes', {
  enumerable: true,
  get: function () {
    return listPalettes;
  }
...
Object.defineProperty(exports, 'mandatoryDisclosure', {
  enumerable: true,
  get: function () {
    return mandatoryDisclosure;
  }
});
Object.defineProperty(exports, 'normalizeDeclaredProvenanceInputs', {
  enumerable: true,
  get: function () {
    return normalizeDeclaredProvenanceInputs;
  }
});
Object.defineProperty(exports, 'normalizeDeclaredProvenanceValue', {
  enumerable: true,
  get: function () {
    return normalizeDeclaredProvenanceValue;
  }
});
Object.defineProperty(exports, 'provenanceParamConstraintError', {
  enumerable: true,
  get: function () {
    return provenanceParamConstraintError;
  }
});
Object.defineProperty(exports, 'provenanceVerificationForContract', {
  enumerable: true,
  get: function () {
    return provenanceVerificationForContract;
  }
});
...
Object.defineProperty(exports, 'requiresHonestyCaption', {
  enumerable: true,
  get: function () {
    return requiresHonestyCaption;
  }
});
Object.defineProperty(exports, 'sampleColormap', {
  enumerable: true,
  get: function () {
    return sampleColormap;
  }
});
Object.defineProperty(exports, 'skillParamsJsonSchema', {
  enumerable: true,
  get: function () {
    return skillParamsJsonSchema;
  }
});
Object.defineProperty(exports, 'toPortableJsonSchema', {
  enumerable: true,
  get: function () {
    return toPortableJsonSchema;
  }
});
Object.defineProperty(exports, 'validateHostRendererInvocation', {
  enumerable: true,
  get: function () {
    return validateHostRendererInvocation;
  }
});
Object.defineProperty(exports, 'validateHostRendererSpec', {
  enumerable: true,
  get: function () {
    return validateHostRendererSpec;
  }
});
Object.defineProperty(exports, 'validatePalette', {
  enumerable: true,
  get: function () {
    return validatePalette;
  }
});
Object.defineProperty(exports, 'validateSkillInvocation', {
  enumerable: true,
  get: function () {
    return validateSkillInvocation;
  }
});
Object.defineProperty(exports, 'validateSkillParams', {
  enumerable: true,
  get: function () {
    return validateSkillParams;
  }
});
...
Object.defineProperty(exports, 'validateVizSpec', {
  enumerable: true,
  get: function () {
