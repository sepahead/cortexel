...
const DECLARED_INPUTS_PORTABLE_SCHEMA = Object.freeze({
	...
	propertyNames: Object.freeze({
		...
		allOf: Object.freeze([Object.freeze({ pattern: "^\\S(?:[\\s\\S]*\\S)?$" }), Object.freeze({ pattern: SAFE_DISPLAY_STRING_PATTERN.source })])
	}),
...
const normalizedRecordKey$1 = z.string().min(1).max(80).regex(/^\S(?:[\s\S]*\S)?$/, "record keys must already be trimmed and contain a non-whitespace character").regex(SAFE_DISPLAY_STRING_PATTERN, "record keys must not contain display control characters");
/**
...
const JsonParamsSchema = z.unknown().transform((params, ctx) => {
...
const ProvenanceSchema = z.object({
	...
	calibrated_posterior: z.literal(false).default(false),
	...
	*  producer's responsibility. */
	declared_inputs: JsonParamsSchema.pipe(z.record(normalizedRecordKey$1, z.union([
		z.string().max(5e3).regex(SAFE_DISPLAY_STRING_PATTERN),
		z.number(),
		z.literal(true)
...
const VizSpecSchema = z.object({
	...
	*  cannot recover the skill — this field closes that gap. */
	skill: z.string().trim().min(1).max(80).regex(SAFE_DISPLAY_STRING_PATTERN, "skill must not contain display control characters").optional(),
	...
	*  a dev-mode warning). When absent, the host's active palette is used. */
	palette: z.string().trim().min(1).max(60).regex(SAFE_DISPLAY_STRING_PATTERN, "palette must not contain display control characters").optional(),
...
function validateVizSpec(input) {
	try {
		...
		return {
			ok: false,
			errors: [`(root): validation could not safely inspect the payload: ${safeErrorMessage(error)}`]
		};
	}
}
...
function composeHonestyCaption(p, contractDisclosures = {}) {
	...
	if (note) parts.push(`Caller note (unverified): ${safeDiagnosticText(note, 500)}`);
...
const ProvenanceKeyEnum = z.enum(PROVENANCE_KEYS);
...
function canonicalJsonParamError(constraint, actual, params) {
	...
	try {
		if (constraint.allowDigest && isCanonicalDigest(actual) && actual === canonicalDigest(resolved.value)) return null;
...
function projectedCollectionError(constraint, actual, params) {
	...
	if (constraint.relation === "equals" && constraint.allowDigest && isCanonicalDigest(actual)) return actual === canonicalDigest(digestCollection) ? null : `${constraint.provenanceKey} digest must match the projected params collection in checked encounter order`;
	...
	if (constraint.relation === "equals") return canonicalize(declaredCollection) === canonicalize(expected) ? null : `${constraint.provenanceKey} must equal the projected params collection`;
...
const gpuNumber = z.number().min(-34028234663852886e22, "value exceeds the finite Float32 range used by render buffers").max(FLOAT32_MAX$1, "value exceeds the finite Float32 range used by render buffers");
const gpuArray = z.array(gpuNumber).max(PARAM_LIMITS.maxSamples);
const idArray = z.array(z.number().int("node/sender ids must be integers").nonnegative("node/sender ids must be non-negative").max(Number.MAX_SAFE_INTEGER, "node/sender ids must be safe integers")).max(PARAM_LIMITS.maxSamples);
const displayText = (max) => z.string().trim().min(1).max(max).regex(SAFE_DISPLAY_STRING_PATTERN, "display text must not contain control or bidi characters").meta({ "x-cortexel-normalize": "trim" });
...
const Rfc3339TimestampSchema = z.iso.datetime({ offset: true }).max(80).regex(/T\d{2}:\d{2}:\d{2}(?:\.\d+)?(?:Z|[+-]\d{2}:\d{2})$/, "timestamp must be RFC 3339 with seconds and an explicit UTC/numeric offset");
const units = displayText(80);
const normalizedRecordKey = z.string().min(1).max(80).regex(/^\S(?:[\s\S]*\S)?$/, "record keys must already be trimmed and contain a non-whitespace character").regex(SAFE_DISPLAY_STRING_PATTERN, "record keys must not contain control or bidi characters");
...
const VoltageTraceParamsSchema = z.object({
	times_ms: timeArray.min(2),
	series: z.array(gpuArray.min(1)).min(1).max(PARAM_LIMITS.maxSeries),
	series_labels: z.array(displayText(120)).min(1).max(PARAM_LIMITS.maxSeries),
...
const SpikeRasterParamsSchema = z.object({
...
const IsiDistributionParamsSchema = z.object({
...
const PsthParamsSchema = z.object({
	bin_centers_ms: timeArray.min(1),
	values: gpuArray.min(1),
	bin_width_ms: z.number().positive().max(Number.MAX_VALUE),
	normalization: z.enum([
...
const PopulationRateSeriesSchema = z.object({
	id: displayText(120),
	label: displayText(240),
	recorded_sender_count: z.number().int("recorded_sender_count must be an integer").positive("recorded_sender_count must be positive").max(Number.MAX_SAFE_INTEGER, "recorded_sender_count must be a safe integer"),
	spike_counts: z.array(z.number().int("spike counts must be integers").nonnegative("spike counts cannot be negative").max(Number.MAX_SAFE_INTEGER, "spike counts must be safe integers")).min(1).max(PARAM_LIMITS.maxSamples),
...
const PopulationRateParamsSchema = z.object({
	bin_centers_ms: timeArray.min(1),
	bin_width_ms: z.number().positive().max(Number.MAX_VALUE),
	window_start_ms: z.number(),
	window_stop_ms: z.number(),
	series: z.array(PopulationRateSeriesSchema).min(1).max(PARAM_LIMITS.maxSeries),
...
const RateResponseParamsSchema = z.object({
...
const topologyCount = z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).refine((value) => !Object.is(value, -0), "topology counts and ranks must not be negative zero");
const topologyPositiveCount = topologyCount.positive();
const MpiTargetRankLocalScopeSchema = z.object({
...
const MpiRankLocalPositionScopeSchema = z.object({
...
const PositionScopeSchema = z.discriminatedUnion("kind", [
	z.object({ kind: z.literal("single_process_complete") }).strict(),
...
const ConnectionGraphNodeSchema = z.object({
	id: idArray.element,
	label: displayText(120)
}).strict();
const ConnectionGraphEdgeSchema = z.object({
	id: displayText(240),
...
const ConnectionGraphParamsSchema = z.object({
...
const MatrixCellBaseSchema = z.object({
...
const AdjacencyMatrixParamsSchema = z.object({
	...matrixBaseShape,
...
const WeightMatrixParamsSchema = z.object({
	...matrixBaseShape,
...
const DelayMatrixParamsSchema = z.object({
	...matrixBaseShape,
...
const DelayDistributionParamsSchema = z.object({
	bin_centers_ms: timeArray.min(1),
...
const SpatialMap2DNodeSchema = z.object({
	id: idArray.element,
	label: displayText(120),
	x: gpuNumber,
	y: gpuNumber
...
const SpatialMap2DParamsSchema = z.object({
...
const WeightHistogramParamsSchema = z.object({
...
const Spatial3DObjectSchema = z.object({
	x: gpuNumber,
	y: gpuNumber,
	z: gpuNumber
}).passthrough();
const Spatial3DParamsSchema = z.object({
	objects: z.array(Spatial3DObjectSchema).min(1).max(PARAM_LIMITS.maxSpatialObjects),
	coordinate_units: units
}).strict();
const PlasticityParamsSchema = z.object({
	times_ms: timeArray.min(1),
...
const PhasePlaneParamsSchema = z.object({
	grid: z.record(normalizedRecordKey, gpuArray.min(2)).refine((g) => Object.keys(g).length === 2, { message: "phase-plane grid must declare exactly two state-variable axes with at least two coordinates each" }),
	...
	axis_order: z.tuple([normalizedRecordKey, normalizedRecordKey]).refine(([first, second]) => first !== second, { message: "axis_order must name two distinct state variables" }),
...
const AstrocyteParamsSchema = z.object({
	times_ms: timeArray.min(2),
...
const KnowledgeGraphAttributeScalarSchema = z.union([
	z.string().max(KNOWLEDGE_GRAPH_LIMITS.maxAttributeStringLength).regex(SAFE_DISPLAY_STRING_PATTERN, "attribute strings must not contain control or bidi characters"),
	z.number(),
	z.boolean(),
	z.null()
]);
const KnowledgeGraphAttributeValueSchema = z.union([KnowledgeGraphAttributeScalarSchema, z.array(KnowledgeGraphAttributeScalarSchema).max(KNOWLEDGE_GRAPH_LIMITS.maxAttributeArrayItems)]);
const KnowledgeGraphAttributeKeySchema = normalizedRecordKey.max(KNOWLEDGE_GRAPH_LIMITS.maxAttributeKeyLength);
const KnowledgeGraphAttributesSchema = z.record(KnowledgeGraphAttributeKeySchema, KnowledgeGraphAttributeValueSchema).superRefine((value, ctx) => {
	if (Object.keys(value).length > KNOWLEDGE_GRAPH_LIMITS.maxAttributes) ctx.addIssue({
		code: z.ZodIssueCode.custom,
...
const KnowledgeGraphEvidenceRefSchema = z.discriminatedUnion("kind", [
	...
	z.object({
		...
		page: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).refine((value) => !Object.is(value, -0), "page must not be negative zero").optional(),
		locator: displayText(KNOWLEDGE_GRAPH_LIMITS.maxLocatorLength).optional(),
		excerpt: displayText(KNOWLEDGE_GRAPH_LIMITS.maxExcerptLength).optional(),
...
const KnowledgeGraphEvidenceRefsSchema = z.array(KnowledgeGraphEvidenceRefSchema).min(1).max(KNOWLEDGE_GRAPH_LIMITS.maxEvidenceRefsPerElement).superRefine((evidence, ctx) => {
...
const KnowledgeGraphUncalibratedScoreSchema = z.object({
	kind: z.enum([
...
const DerivedAdvisoryEpistemicSchema = z.object({
...
const KnowledgeGraph3DParamsSchema = z.object({
	...
	nodes: z.array(KnowledgeGraphNodeSchema).min(1).max(PARAM_LIMITS.maxGraphNodes),
	...
	value.edges.forEach((edge, index) => {
		...
		if (parallelCount > KNOWLEDGE_GRAPH_LIMITS.maxParallelEdgesPerPair) addIssue({
			code: z.ZodIssueCode.custom,
			path: ["edges", index],
...
const Spatial2DParamsSchema = z.object({
	positions: z.array(z.tuple([gpuNumber, gpuNumber])).min(1).max(PARAM_LIMITS.maxSpatialObjects),
	coordinate_units: units
}).strict();
const CorrelogramPairSchema = z.object({
	reference_label: displayText(240),
	target_label: displayText(240)
}).strict();
const CorrelogramStatisticSchema = z.discriminatedUnion("kind", [
	z.object({
		kind: z.literal("raw_pair_count"),
		units: z.literal("count")
	}).strict(),
	z.object({
		kind: z.literal("weighted_pair_sum"),
		units
	}).strict(),
	z.object({
...
const CorrelogramParamsSchema = z.object({
...
const StimulusResponseParamsSchema = z.object({
	times_ms: timeArray.min(1),
...
const CompartmentalParamsSchema = z.object({
	times_ms: timeArray.min(2),
...
const AnimationReplayParamsSchema = z.object({ frames: z.array(z.object({
	time_ms: z.number().nonnegative(),
	state: z.record(normalizedRecordKey, z.unknown()).refine((state) => Object.keys(state).length > 0, { message: "frame state must contain at least one field" }),
...
function toPortableJsonSchema(schemaSource) {
	const schema = z.toJSONSchema(schemaSource, { io: "input" });
	...
	switch (skillId) {
		...
		case "corpus.knowledge_graph": {
			...
			const attributesAreBounded = (value) => {
				...
				if (!attributes || Object.keys(attributes).length > KNOWLEDGE_GRAPH_LIMITS.maxAttributes) return false;
				return Object.values(attributes).every((attribute) => !Array.isArray(attribute) || attribute.length <= KNOWLEDGE_GRAPH_LIMITS.maxAttributeArrayItems);
			};
		...
		for (let index = 0; index < length; index++) {
			...
			if (attributes !== null && typeof attributes === "object" && !Array.isArray(attributes)) {
				...
				for (const attributeKey in attributes) {
					...
					attributeCount += 1;
					if (attributeCount > KNOWLEDGE_GRAPH_LIMITS.maxAttributes) return {
					...
					const valueIssue = tooLongValue(attributeDescriptor.value, `${key}.${index}.attributes.${attributeKey}`, KNOWLEDGE_GRAPH_LIMITS.maxAttributeArrayItems);
			...
			const evidenceIssue = tooLongValue(evidence, `${key}.${index}.evidence`, KNOWLEDGE_GRAPH_LIMITS.maxEvidenceRefsPerElement);
...
const HostRendererInvocationSchema = z.object({
	skill: z.string().trim().min(1).max(80).regex(SAFE_DISPLAY_STRING_PATTERN, "skill must not contain display control characters"),
	specVersion: z.literal(CORTEXEL_SPEC_VERSION).optional(),
...
function safeAuthoringPath(path, field) {
	return safeDiagnosticText(field === void 0 ? path : `${path}.${safeAuthoringField(field)}`, PUBLIC_DIAGNOSTIC_LIMITS.maxPathLength);
}
