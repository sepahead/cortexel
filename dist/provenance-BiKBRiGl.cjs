...
function resolveBudgetProfile(options) {
	let requested = require_contract_identity.DEFAULT_PROFILE;
	try {
	...
	return {
		...
		limits: require_contract_identity.tryGetBudgetLimits(requested)
	};
}
/** Read the request profile only from an already-owned plain JSON tree. */
function requestedBudgetProfile(value) {
	if (value === null || typeof value !== "object" || Array.isArray(value)) return require_contract_identity.DEFAULT_PROFILE;
	const presentation = value.presentation;
	if (presentation === null || typeof presentation !== "object" || Array.isArray(presentation)) return require_contract_identity.DEFAULT_PROFILE;
	return Object.prototype.hasOwnProperty.call(presentation, "budgetProfile") ? presentation.budgetProfile : require_contract_identity.DEFAULT_PROFILE;
}
...
function captureRawRequestInput(text, options = {}) {
	...
	const effective = require_contract_identity.trySelectTighterBudgetProfile(host.profile, requested);
...
//# sourceMappingURL=provenance-BiKBRiGl.cjs.map